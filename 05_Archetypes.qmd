...
Save PWMs for ortholog-assigned CisBP motifs
...
universalmotif::write_meme(pwms_uniq, file.path(arc_dir, "input-pwms.meme"), overwrite = TRUE)
...
Calculate pairwise similarities (PCC) between input motifs.
...
Alternatively, we can use JSD similarities calculated in `05_Archetypes_JSD.ipynb`.

```{r}
if (method == "PCC") {

  # load motifs PCC similarity
  ...
  # load patterns pairwise JSD similarity
  jsd_dat <- fread(
    file.path(arc_dir, "motif-similarity-JSD.tsv.gz")
  )
  jsd_dcs <- dcast.data.table(jsd_dat, motif1 ~ motif2, value.var = "JSD")
  stopifnot(all.equal(sort(jsd_dcs[[1]]), sort(colnames(jsd_dcs)[-1])))
  jsd_mat <- as.matrix(jsd_dcs[, -1])
  rownames(jsd_mat) <- jsd_dcs[[1]]
  stopifnot(all.equal(rownames(jsd_mat), colnames(jsd_mat)))

  # convert to similarities
  sim_mat <- 1 / jsd_mat
  sim_max <- max(sim_mat[!is.infinite(sim_mat)], na.rm = TRUE)
  sim_mat <- sim_mat / sim_max
  sim_mat[is.infinite(sim_mat)] <- 1
  diag(sim_mat) <- 1
  range(sim_mat)
}
```

Order similarity matrix, then choose the minimum cluster
...
hc <- hclust(tgs_dist(jsd_mat), method = hclust_method)

pdf(
  file.path(fig_dir, sprintf("motif-input-similarity-%s-hc-%s.pdf", method, hclust_method)),
  width = 92, height = 12
)
plot(hc)
dev.off()
...
- inspect different similarity matrices

```{r}
sim_mat_pcc <- readRDS(file.path(arc_dir, "motif-similarity-PPM-PCC.rds"))
sim_mat_jsd <- readRDS(file.path(arc_dir, "motif-similarity-PPM-JSD.rds"))
jsd_mat_jsd <- jsd_mat
rn <- ord
cn <- ord

pdf(file.path(fig_dir, "motif-input-similarity-mats.pdf"), width = 12, height = 12)

Heatmap(
  sim_mat_pcc[rn, cn], name = "PCC_similarity",
  show_row_names = FALSE, show_column_names = FALSE, 
  cluster_rows = FALSE, cluster_columns = FALSE,
  use_raster = TRUE
)
Heatmap(
  sim_mat_jsd[rn, cn], name = "JSD_similarity",
  show_row_names = FALSE, show_column_names = FALSE, 
  cluster_rows = FALSE, cluster_columns = FALSE,
  use_raster = TRUE
)
Heatmap(
  jsd_mat_jsd[rn, cn], name = "JSD",
  show_row_names = FALSE, show_column_names = FALSE,
  cluster_rows = FALSE, cluster_columns = FALSE, 
  use_raster = TRUE
)
...
Initial clusters for archetyping
...
mt_cls <- mt_meta[, .N, cluster][order(-N)]$cluster
mt_meta[, cluster := factor(cluster, levels = mt_cls)]
setorder(mt_meta, cluster)
mt_meta[, .N, cluster]
fwrite(
  mt_meta,
...
- inspect similarity clusters

```{r}
pdf(file.path(
  fig_dir, "input-motifs-clusters.pdf"
), width = 8, height = 46)

cls <- unique(mt_meta$cluster)
for (x in seq_along(cls)) {

  mts <- mt_meta[cluster == cls[x]]$motif_name
  print(sprintf("%s/%s %s motifs", x, length(cls), length(mts)))

  # motifs
  ppms <- pwms_uniq[mts]
  if ("universalmotif" %in% class(ppms)) ppms <- list(ppms)
  ppms <- lapply(ppms, function(m) {
    m@alphabet <- "DNA"
    m
  })

  # plot logos
  mots <- view_motifs(ppms)

  # add spacer for small number of motifs
  np <- length(ppms)
  if (np < 40) {
    mots <- mots + plot_spacer() + plot_layout(ncol = 1, heights = c(np, 20-np))
  }

  tryCatch({
    print(mots)
  }, error = function (e) message(sprintf("Failed to plot cluster %s\n%s", x, e)))
}
dev.off()
...
```{r message=FALSE}
# similarities
sim_mat <- readRDS(file.path(arc_dir, 
  sprintf("motifs-similarity-%s-%s.rds", similarity, method)
))

# motif clusters
mt_meta <- fread(file.path(arc_dir, "input-motifs-meta.tsv"))
ctr <- mt_meta[match(rownames(sim_mat), motif_name)][, setNames(cluster, motif_name)]

# motif pwms
pwms_uniq <- readRDS(file.path(arc_dir, "input-pwms.rds"))
...
# add names
names(arch) <- paste("ARCH", seq_along(arch), sep = "")
...
arch_meta <- data.table(
  archetype = names(arch),
  num_motifs = sapply(arch, function(x) length(x$ppms)),
  ic = sapply(arch, function(x) x$ic_consensus@icscore),
  len = sapply(arch, function(x) dim(x$ic_consensus@motif)[2])
)
mots_meta <- rbindlist(lapply(names(arch), function(x) {
  ppms <- arch[[x]]$ppms
  if ("universalmotif" %in% class(ppms)) ppms <- list(ppms)
  mots <- sapply(ppms, function(y) y@name)
  data.table(motif_name = mots)[, archetype := x]
}))
arch_meta <- merge.data.table(mots_meta, arch_meta, by = "archetype", all.x = TRUE, sort = FALSE)
arch_meta <- merge.data.table(arch_meta, mt_meta, by = "motif_name", all.x = TRUE, sort = FALSE)
fwrite(
  mt_meta,
  file.path(arc_dir, "input-motifs-archetypes-meta.tsv"),
  sep = "\t"
)
```

Annotate archetype motifs i.e. create a dictionary with motif-to-archetype mappings and other metadata.
...
# similarities
sim_mat_file <- file.path(arc_dir, sprintf(
  "motif-similarity-%s-%s.rds", similarity, method
))
...
```{r}
pdf(file.path(
  fig_dir, basename(str_replace(arch_file, "\\.rds$", "-archetypes-logo-filled.pdf"))
), width = 8, height = 26)
for (x in names(arch)) {
  print(sprintf("%s/%s", x, length(arch)))

  # pattern motifs
  patts_ppm <- arch[[x]]$ppms
  if ("universalmotif" %in% class(patts_ppm)) patts_ppm <- list(patts_ppm)
  patts_ppm <- lapply(patts_ppm, function(m) {
    m@alphabet <- "DNA"
    m
  })

  # archetype
  motif_ppm <- arch[[x]]$ppm_consensus
  motif_ppm@alphabet <- "DNA"
  motif_ppm@name <- x
  
  # plot logos
  mots <- view_motifs(c(motif_ppm, patts_ppm))
...
# plot upset archetypes
require(ComplexUpset)
arch_meta_ups <- dcast.data.table(
  unique(dict[, .(archetype_name, source)][, x := TRUE]), archetype_name ~ source, value.var = "x"
)
arch_meta_ups[is.na(arch_meta_ups)] <- FALSE
gp_ups <- upset(
  arch_meta_ups,
  names(category_cols)[1:3],
  set_sizes=FALSE,
  stripes=unname(category_cols)[1:3],
  matrix=(
          intersection_matrix(
            geom = geom_point(
              size = 5,
          ...
          scale_color_manual(
            values=c('TRUE'='#808080', 'FALSE'='#ffffff'),
            labels=c('TRUE'='yes', 'FALSE'='no'),
            breaks=c('TRUE', 'FALSE'),
            name="In the archetype"
          )
  )
)
gp_file <- file.path(
  fig_dir,
  sprintf("motif-archetypes-source-upset-%s.pdf", arc_id)
)
ggsave(gp_file, gp_ups, width = 5, height = 4)
...
similarity <- "PPM"
method <- "PCC"
arc_id <- sprintf("%-%s-0.8-IC0.5-5bp", similarity, method)
...
Score input motifs for archetypes in consensus set of peaks.
...
Score orthology-assigned motifs in consensus set of peaks.
...
# motifs
mots_mta_lst <- readRDS(file.path(cisbp_dir, "cisbp_nvec_orthology_motifs.rds"))
mona_mta_lst <- mta_convert_umot_to_monalisa(mots_mta_lst)
maxs_mta <- sapply(
  names(mots_mta_lst),
  function(x) maxScore(convert_type(mots_mta_lst[[x]], "PWM")@motif),
...
saveRDS(
  mta_scores_mona,
  file.path(arc_dir, sprintf("motif-scores-orthology-motifs-mona-q%s.rds", q))
)
...
fwrite(
  mta_data,
  file.path(arc_dir, sprintf("motif-scores-orthology-motifs-mona-q%s.tsv.gz", q)),
  sep = "\t"
)
```
...
Calculate archetype motif enrichment in cell type specific peaks.
...
fwrite(
  enr_dt,
  file.path(arc_dir, sprintf("motif-enrichment-cell-type-orthology-%s-mona-q%s.tsv.gz", arc_id, q)),
  sep = "\t"
)
...
Only plot archetypes enrichment for SFig.
...
# subset archetypes
sig_arc <- unique(arc_dt[(padj < 0.00001 & fc > 2)]$motif)
length(sig_arc)
top_enr_sig <- arc_dt[motif %in% sig_arc]

# transform p value for plotting
top_enr_sig[, padj_scaled := -1 * log10(padj)]

# scale values for plotting
top_enr_sig[, padj_scaled := pmin(padj_scaled, 60)]
top_enr_sig[, fc_scaled := pmin(fc, 6)]

# order cell types
top_enr_sig[cell_type == "neuronal_gastrula", cell_type := "neuronal"]
stopifnot(all(top_enr_sig$cell_type %in% cell_types))
top_enr_sig[, cell_type := factor(cell_type, levels = cell_types)]

# select representative motifa
top_enr_sig[, motif_name := str_remove(motif, "ARCH\\d+_")]
setorder(top_enr_sig, motif_name, -padj_scaled)
top_enr_mots <- top_enr_sig[, .SD[1], .(motif_name)]$motif
top_enr_sig <- top_enr_sig[motif %in% top_enr_mots]

# construct more informative motif names
dict_nms <- unique(dict[,.(motif=archetype_name, name=tf_cisbp_name)])[name!=""]
dict_nms[, name := paste(sort(unique(.SD$name)), collapse = "/"), motif]
dict_nms <- unique(dict_nms)
top_enr_sig <- merge.data.table(
  top_enr_sig, dict_nms, 
  by = "motif", all.x = TRUE, sort = FALSE
)
top_enr_sig[is.na(name), name := str_remove(motif, "ARCH\\d+_")]

# cluster archetypes
top_arc_fc <- dcast.data.table(
  unique(top_enr_sig[, .(motif, cell_type, padj_scaled)]), 
  motif ~ cell_type, value.var = "padj_scaled"
)
top_arc_fc[is.na(top_arc_fc)] <- 0
top_arc_fc_mat <- as.matrix(top_arc_fc[, -1])
rownames(top_arc_fc_mat) <- top_arc_fc[[1]]
...
# order motifs
top_enr_sig[, motif := factor(motif, levels = top_arc_ord)]
setorder(top_enr_sig, motif)
top_enr_sig[, name := factor(name, levels = unique(top_enr_sig$name))]
...
gp_enr <- ggplot(
  ...
  geom_point(
    aes(size = padj_scaled, color = fc_scaled)
  ) +
...
# add source meta
source_dt <- unique(dict[, .(motif=archetype_name, source=source)])
source_enr_dt <- merge.data.table(
  top_enr_sig, source_dt,
  by = "motif", all.x = TRUE, sort = FALSE, allow.cartesian = TRUE
)
source_enr_dt <- unique(source_enr_dt[, .(motif, motif_name, name, source)])
source_enr_dt[grepl("CisBP", source), source := "CisBP"]

# plot motif sources
gp_src <- ggplot(
    source_enr_dt,
    aes(source, name, fill = source)
  ) +
  geom_tile() +
...
# save plot
ggsave(
  file.path("plots", "Figures", "figS4-motifs-enrichment-dotplot-archetype.pdf"),
  gp_enr + gp_src + plot_layout(guides = "collect", widths = c(8, 1)),
...
Binary matrix of archetype motif hits.

```{r}
# create binary matrix of motif hits in peaks for archetypes
arc_id <- "PPM-PCC-0.8-IC0.5-5bp"
q <- 0.98
...
pks_matx <- peaks_mat_seacells[pks, ]
arc_matx <- arc_matx[pks, ]

# sanity check
all.equal(rownames(arc_matx), as.character(rownames(pks_matx)))
```

Binary matrix of orthology assigned motifs hits.

```{r}
# create binary matrix of motif hits in peaks for orthology motifs
q <- 0.98
mta_hits <- unique(fread(
  file.path(arc_dir, sprintf("motif-scores-orthology-motifs-mona-q%s.tsv.gz", q))
)[, .(motif, peak, motif_score)])
setorder(mta_hits, -motif_score)
mta_hits <- mta_hits[, .SD[1], .(motif, peak)]
mta_data <- dcast.data.table(mta_hits, peak ~ motif, value.var = "motif_score")

# convert to matrix
mta_matx <- data.matrix(mta_data[, -1])
mta_matx[!is.na(mta_matx)] <- 1
mta_matx[is.na(mta_matx)] <- 0
rownames(mta_matx) <- mta_data[[1]]
mta_matx <- as(mta_matx, "dgCMatrix")

# only motifs assigned to expressed  genes
mort <- fread(file.path("data", "Nvec_predicted_motifs.tsv"))
setnames(mort, "Query", "gene")
ctfp_mat <- readRDS(file.path(
  "results", "Metacells", "metacell", 
  "gene_expression_fc_genes_exp_FC2_acc_FC4_spearman.rds"
))
mts <- mort[gene %in% rownames(ctfp_mat)]$Predicted_motif
mta_matx <- mta_matx[, colnames(mta_matx) %in% mts]

# include only peaks present in both matrices
pks <- intersect(rownames(peaks_mat_seacells), rownames(mta_matx))
pks_matx <- peaks_mat_seacells[pks, ]
mta_matx <- mta_matx[pks, ]

# sanity check
all.equal(rownames(mta_matx), as.character(rownames(pks_matx)))
```
...
peaks <- fread(file.path(pks_dir, "Peaks_cell_type_mapped.tsv.gz"))
...
bkgd_adult_pks <- apply(assay(bkgd_adult), 2, function(x) {
  ...
  match(peaks_mappd, rownames(pks_matx))
})
...
sample_replace <- ifelse(nrow(bgd_peaks) < nrow(pks_matx), TRUE, FALSE)
bgd_peaks <- bgd_peaks[sample(
  seq_len(nrow(bgd_peaks)), nrow(pks_matx), 
...
ChromVAR calculation for archetypes.
...
chromvar_deviations <- .customDeviations(
  countsMatrix = pks_matx,
  annotationsMatrix = arc_matx,
  backgroudPeaks = bgd_peaks,
  expectation = rowSums(pks_matx) / sum(rowSums(pks_matx)),
  prefix = "SEACells",
  out = c("deviations", "z"),
  threads = 1,
  verbose = TRUE
...
ChromVAR calculation for orthology assigned motifs.
...
Calculate correlation of gene scores and gene expression with motif deviation Z scores:
...
# motif chromVAR deviation scores
chromvar_mat <- readRDS(file.path(
  arc_dir, "ChromVAR-PPM-PCC-0.8-IC0.5-5bp", "ArchetypesSEACellsMatrix", "Matrix-Archetypes-SEACells.rds"
))
...
# calculate gene score correlations
...
- orthology based motifs

```{r}
# motif chromVAR deviation scores
chromvar_mat <- readRDS(file.path(
  mta_dir, "ChromVAR-orthology-motifs", "MotifsSEACellsMatrix", "Matrix-Motifs-SEACells.rds"
))
...
saveRDS(
  mot_gen_cor,
  file.path(
    mta_dir, "ChromVAR-orthology-motifs",
    "correlation-seacells-motif-score-gene-score.rds"
  )
)
