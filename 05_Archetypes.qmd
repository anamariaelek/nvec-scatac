...
saveRDS(
  arc_scores_mona,
  str_replace_all(arch_fn, c("motif" = "motif-scores", "pwms.rds" = "mona.rds"))
)
...
fwrite(
  arc_data,
  str_replace_all(arch_fn, c("motif" = "motif-scores", "pwms.rds" = "mona.tsv.gz")),
  sep = "\t"
)
...
arc_scores_mona <- readRDS(
  file.path(arc_dir, sprintf("motif-scores-archetypes-%s-mona.rds", arc_id))
)
...
# add motif metadata
tf_fam_dt <- unique(dict[, .(archetype_name, tf_family, source)])#[tf_family != ""]
#tf_fam_dt[, tf_family := paste(sort(unique(.SD[tf_family != ""]$tf_family)), collapse = ","), archetype_name]
#tf_fam_dt[, source := paste(sort(unique(.SD[source != ""]$source)), collapse = ","), archetype_name]
...
dt_b2 <- unique(g_dt[, .(gene, group, archetype_name)][
...
dt_b3 <- g_dt[, .(gene, group, archetype_name, archetype_family, tf_family)][
  group == "none"][
...
agn_dt <- assign_dt[,.(gene, archetype_name)]
...
arc_plt_dt[, minuslog10padj_scaled := pmin(minuslog10padj, 60)]
...
ggsave(
  ...
  arc_ann_gp,
  height = 22, width = 14, limitsize = FALSE
)
```
...
# save
ggsave(
  file.path(fig_dir, sprintf("motif-activity-gene-expression-FC-%s-padj-%s.pdf", fc_thr, pa_thr)),
  gen_plt_gp,
