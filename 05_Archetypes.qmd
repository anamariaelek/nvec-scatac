...
Similarities between archetypes
...
# # construct gene labels
# arc_ann_dt[, gene_label := ""]
# arc_ann_dt[gene != "", gene_label := str_remove(gene, "Nvec_(vc1.1_)*")]
# arc_ann_dt[pfam != "", gene_label := sprintf("%s | %s", ifelse(nchar(pfam)>25, paste0(substr(pfam, 1, 22), "..."), pfam), gene_label)]
# arc_ann_dt[og != "", gene_label := sprintf("%s | %s", ifelse(nchar(og)>25, paste0(substr(og, 1, 22), "..."), og), gene_label)]
# arc_ann_dt[common_name != "", gene_label := sprintf("%s | %s", common_name, gene_label)]

# cluster motifs
arc_ann_dt[, cell_type := factor(cell_type, levels = cell_types)]
arc_ann_dt[, minuslog10padj := -1 * log10(padj)]
padj_dt <- unique(arc_ann_dt[, .(archetype_name, cell_type, minuslog10padj)])
...
arc_ann_dt[, archetype_name := factor(archetype_name, levels = arc_lvl)]
setorder(arc_ann_dt, archetype_name)

# data to plot
arc_plt_dt <- arc_ann_dt[, .(cell_type, archetype_name, minuslog10padj, fc, gene, common_name, og, pfam)]
...
arc_ann_gp <- ggplot(
  ...
  scale_y_discrete(
    breaks = arc_plt_dt$archetype_name,
    labels = arc_plt_dt$gene_label
  ) +
  scale_fill_gradientn(
...
# save
ggsave(
  file.path(fig_dir, "motif-enrichment.pdf"),
  arc_ann_gp,
  height = 54, width = 14, limitsize = FALSE
)
```
