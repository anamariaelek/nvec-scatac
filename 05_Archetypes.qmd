...
peaks <- fread(file.path(
  pks_dir, "scATAC_reduced_peaks_gene_assignment_coaccess.tsv"
))
...
saveRDS(
  mta_scores_mona,
  file.path(arc_dir, "motif-scores-input-motifs-unique-mona.rds")
)
...
fwrite(
  arc_data,
  file.path(arc_dir, "motif-scores-archetypes-mona.tsv"),
  sep = "\t"
)
...
fwrite(
  mta_data,
  file.path(arc_dir, "motif-scores-input-motifs-unique-mona.tsv"),
  sep = "\t"
)
```
...
mta_data <- fread(file.path(
  arc_dir, "motif-scores-input-motifs-unique-mona.tsv"
))
arc_data <- fread(file.path(
  arc_dir, "motif-scores-archetypes-unique-mona.tsv"
))
dict <- fread(file.path(
  arc_dir, "motif-archetypes-PPM-PCCnorm-0.8-IC0.5-8bp-unique.dict"
))
...
arc_sco <- arc_sco[, .SD[1], .(motif, peak)][
  , .(motif, peak, motif_score_norm)]
setnames(
  arc_sco,
  c("motif", "motif_score_norm"),
  c("archetype_name", "archetype_score_norm")
)
...
# complete cases
all_sco[is.na(archetype_score_norm), archetype_score_norm := 0]
all_sco[is.na(motif_score_norm), motif_score_norm := 0]
compara_sco <- all_sco[!is.na(archetype_score_norm) & !is.na(motif_score_norm)]
...
gp_sco <- ggplot(
    ...
    aes(motif_score_norm, archetype_score_norm)
  ) +
...
ggsave(
  file.path(fig_dir, "motif-scores-compara-archetype-unique.pdf"),
  gp_sco,
  width = 6, height = 6
)
```

Calculate cell type enrichment in specific peaks
...
enr_dt <- rbindlist(lapply(cell_types, function(ct) {
  ...
  # cell type peaks
  pks_fn <- file.path(
    res_dir_atac, "ArchRProj", "PeakDifferential", "cell_type_filtered",
  ...
  rbindlist((list("archetype" = arc_dt, "motif" = mta_dt)), idcol = "group")
}))

# save results
fwrite(
  enr_dt[group == "archetype"][, group := NULL],
  file.path(arc_dir, "motif-enrichment-cell-type-archetypes-all.tsv"),
  sep = "\t"
)
fwrite(
  enr_dt[group == "motif"][, group := NULL],
  file.path(arc_dir, "motif-enrichment-cell-type-input-motifs-unique.tsv"),
  sep = "\t"
)
```
...
all_enr <- merge.data.table(
  mta_dt, arc_dt, by = c("archetype_name", "label"),
  all = TRUE, sort = FALSE
)
...
# # plot dotmaps for motifs and archetypes per cell type
# top_enr_padj <- melt.data.table(
#   unique(top_enr[, .(archetype_name, motif, padj_archetype, padj_motif, cell_type)]), 
#   measure.vars=c("padj_archetype","padj_motif"), 
#   variable.name="type", value.name = "padj"
# )
# top_enr_padj[, type := str_extract(type, "motif|archetype")]
# top_enr_fc <- melt.data.table(
#   unique(top_enr[, .(archetype_name, motif, fc_archetype, fc_motif, cell_type)]),
#   measure.vars=c("fc_archetype","fc_motif"), variable.name="type", value.name = "fc")
# top_enr_fc[, type := str_extract(type, "motif|archetype")]
# top_enr_melt <- merge.data.table(
#   top_enr_padj, top_enr_fc,
#   by = intersect(colnames(top_enr_padj),colnames(top_enr_fc)),
#   all = TRUE, sort = FALSE
# )
# top_enr_melt[, archetype_motif := paste(archetype_name, motif, sep = " | ")]
...
```{r}
# I want more informative cisbp names
cisbp <- fread(file.path(
  cisbp_dir, "motifs_cisbp_all_2022-03-09", "TF_Information_all_motifs.txt"
))
cisbp <- cisbp[TF_Status == "D"]
cisbp <- unique(cisbp[, .(TF_Name, Motif_ID)])
setnames(cisbp, c("Motif_ID", "TF_Name"), c("motif","motif_name"))
top_enr <- all_enr[, .SD[1], .(archetype_name, cell_type)]
top_enr <- merge.data.table(
  top_enr, cisbp, by = "motif", all.x = TRUE, sort = FALSE
)
top_enr[is.na(motif_name), motif_name := motif]
top_enr[, archetype_num := str_extract(archetype_name, "ARCH\\d+")]
top_enr[, motif_archetype_name := paste(archetype_num, motif_name, sep = " || ")]

# subset archetypes
top_enr_sig <- top_enr[(padj_archetype < 0.001 & fc_archetype > 2) | (padj_motif < 0.001 & fc_motif > 2)]
# top_enr_sig <- merge.data.table(
#   top_enr_sig, unique(dict[, .(archetype_name, gene)]), allow.cartesian = TRUE
# )
# top_enr_sig <- rbindlist(list(
#   "unassigned" = top_enr_sig[gene == ""],
#   "assigned" = top_enr_sig[order(-fc_archetype), .SD[1], gene]
# ),  use.names = TRUE)
# top_enr_archs <- top_enr_sig$archetype_name
# top_enr_sig <- top_enr[archetype_name %in% top_enr_archs]

# cluster archetypes
top_arc_fc <- dcast.data.table(
  top_enr_sig, archetype_name ~ cell_type, value.var = "fc_motif"
)
...
top_enr_sig[, archetype_name_short := substr(archetype_name, 1, 60)]
top_enr_sig[, archetype_name_short := factor(
  archetype_name_short,
  levels = unique(top_enr_sig$archetype_name_short)
)]
top_enr_sig[, motif_archetype_name := factor(
  motif_archetype_name,
  levels = unique(top_enr_sig$motif_archetype_name)
)]
mots <- top_enr_sig[
  order(-fc_motif), .SD[1], .(archetype_name, cell_type)
]$motif_name
top_enr_sig <- top_enr_sig[motif_name %in% mots]
top_enr_sig[, motif_name := .SD[1]$motif_name, archetype_name]
top_enr_sig[, motif_name := factor(motif_name, levels = unique(top_enr_sig$motif_name))]
top_enr_sig <- top_enr_sig[
  order(-fc_motif), .SD[1], .(archetype_name, cell_type)
]
...
gp_enr_arc <- ggplot(
    top_enr_sig,
    aes(motif_archetype_name, cell_type)
  ) +
  geom_point(
...
ggsave(
  ...
  gp_enr_arc + gp_enr_mta + plot_layout(guides = "collect"),
  width = 18, height = 86, limitsize = FALSE
)
```
