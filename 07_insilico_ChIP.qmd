...
# peaks order
pks <- rownames(cor_mt)
pks <- unique(paste0("peak",sort(as.integer(str_extract(pks, "\\d+")))))s
stopifnot(all(rownames(cor_mt) %in% pks))
cor_mt <- cor_mt[pks,]
...
stopifnot(all(rownames(sum_clusters) %in% pks))
...
Correlations of chromVAR score and gene expression aggregated per cell type.
...
```{r}
# load metacell level data
chr_exp_ct <- fread(file.path(
  chp_dir,
  sprintf("gene_expression_%s_chromVAR.tsv.gz", exp)
))
if ("archetype_name" %in% colnames(chr_exp_ct)) {
    setnames(chr_exp_ct, "archetype_name", "motif")
}
if ("zscore" %in% colnames(chr_exp_ct)) {
    setnames(chr_exp_ct, "zscore", "motif_deviation")
}
...
), .(gene, gene_name, common_name, pfam, og, motif, cell_type, stage)]
...
if ("motif" %in% colnames(chr_exp_ct) | !"archetype_name" %in% colnames(chr_exp_ct)) {
  setnames(chr_exp_ct, "motif", "archetype_name")
}
...
# gen_plt_dt[common_name == "" & og != "", gene_label := str_trunc(og, 40)]
# gen_plt_dt[gene != "" & common_name == "" & og == "" & pfam != "", gene_label := str_trunc(pfam, 40)]
gen_plt_dt[gene_label == "SoxB2/Sox3" & gene_name != "", gene_label := gene_name]

# order
gen_plt_dt[, row_id := as.integer(factor(gene, levels = unique(gene)))]
gen_plt_dt[, cell_type := factor(cell_type, levels = cell_types)]

# positions for labels
label_positions <- seq(1, max(gen_plt_dt$row_id), length.out = length(unique(gen_plt_dt[gene_label != ""]$gene_label)))
label_data <- unique(
  gen_plt_dt[gene_label != "", .(gene, gene_label, row_id)]
)
label_data[, y_label := label_positions]
label_data[, x_label := min(as.integer(factor(gen_plt_dt$cell_type))) - 0.7]
