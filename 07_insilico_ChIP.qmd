...
mot2gene <- fread(file.path(
    mta_dir, "motif-assignment-archetypes-PPM-PCC-0.8-IC0.5-5bp.tsv.gz"
))
...
cor_mt <- readRDS(file.path(
    chp_dir, lvl, sprintf("peak_TF_expression_correlation_%s.rds", id)
))
...
# save
saveRDS(cor_mt, file.path(chp_dir, lvl, "peak_TF_expression_correlation.rds"))
...
saveRDS(acc_mt, file.path(chp_dir, lvl, "peak_accessibility.rds"))
...
saveRDS(mta_mt, file.path(chp_dir, lvl, "motif_peak_scores.rds"))

# correlation matrix
cor_mt <- cor_mt[, mta_mu$gene]
colnames(cor_mt) <- mta_mu$id
saveRDS(cor_mt, file.path(chp_dir, lvl, "motif_peak_correlation.rds"))
...
agg_counts <- readRDS(file.path(chp_dir, lvl, "peak_accessibility.rds"))
...
# where to save in silico ChI chromVAR matrix
mat_dir <- file.path(
  chp_dir,
  sprintf("insilicoChIP-chromVAR-%s-%s", lvl, id)
)
...
saveRDS(
  bgpks,
  file.path(chp_dir, "Adult_Gastrula_Background_Peaks_SEACell.rds")
)
...
chromvar_ls <- sapply(thrs_list, function(thrs) {
    fread(file.path(
        chp_dir, sprintf("insilicoChIP-chromVAR-%s-%s", lvl, id), thrs,
        "results-chromVAR-insilicoChIP.tsv.gz"
    ))
...
wrap_plots(patch_list, axis_titles = "collect") +   plot_annotation(title = "TF expression vs activity")
dev.off()
```
...
gen_plt_gp <- gp_ctbar_v + gp_dotmap + gp_ctbar_h +
  plot_layout(design = design, widths = c(20, 200), heights = c(200, 10), guides = "collect")
...
ggsave(
  ...
  gen_plt_gp,
  height = 15, width = 8, limitsize = FALSE
)
```
