...
Counts of pairs of enriched motifs in cell type (differential) peaks.
...
pks_dt[is.na(Log2FC), Log2FC := 0]
pks_dt[is.na(Pval), Pval := 1]
pks_dt[is.na(FDR), FDR := 1]
...
print(sprintf(
  "There are %d motif pairs", 
  length(unique(mtd_cta$motif_pair))
))
...
if (nrow(mtd_cta) > 2) {
  ...
  tryCatch({
    mdt_dc <- dcast.data.table(
      unique(mtd_cta[, .(motif_pair, cell_type, frac_peak_pair)]),
...
stopifnot(all(pks_num$cell_type %in% cell_types))
pks_num[, cell_type := factor(cell_type, levels = unique(cell_types))]
ct_gp <- ggplot(pks_num, aes(x = cell_type, y = n_peaks, fill = cell_type)) +
...
# barplot with motif family for pairs
tfam_dt <- unique(melt.data.table(
  mtd_cta[, .(motif_pair, tf_family_motif1, tf_family_motif2)], 
  id.vars = "motif_pair", value.name = "value", variable.name = "variable_old"
))
tfam_dt[, variable_old := str_remove(variable_old, "tf_family_")]
tfam_dt[, value := unique(str_split(value, ";")[[1]]), 1:nrow(tfam_dt)]
setorder(tfam_dt, -value)
tfam_dt[, variable := paste0("motif", 1:nrow(.SD)), motif_pair]
# match name to order of motif in the pair
setnames(tfam_dt, "motif_pair", "motif_pair_old")
tfam_dt[, motif_pair := motif_pair_old]
tfam_dt[variable_old != variable, motif_pair := paste0(
  str_split(motif_pair_old, " \\+ ")[[1]][2],
  " + ",
  str_split(motif_pair_old, " \\+ ")[[1]][1]
), by = .I]
md <- unique(tfam_dt[, .(motif_pair, motif_pair_old)])
motif_pair_vec <- structure(
  as.character(md$motif_pair), names = as.character(md$motif_pair_old)
)
# order by motif pair clustering
# tfam_dt[, motif_pair := factor(motif_pair, levels = unique(mtd_cta$motif_pair))]
# order by motif family
tfam_dt[, tf_family_count := nrow(.SD[variable=="motif1"]), .(value, variable)]
setorder(tfam_dt, variable, tf_family_count)
tfam_dt <- rbindlist(lapply(
  unique(tfam_dt[variable == "motif1"]$value), 
  function(x) {
    dt1 = tfam_dt[variable == "motif1" & value == x]
    y = unique(dt1$motif_pair)
    dt2 = tfam_dt[variable == "motif2" & motif_pair %in% y]
    dt2[, tf_family_count := nrow(.SD), .(value, variable)]
    setorder(dt2, tf_family_count)
    lvl <- unique(dt2$motif_pair)
    dt <- rbind(dt1, dt2)
    dt[, motif_pair := factor(motif_pair, levels = lvl)]
    setorder(dt, motif_pair)
    dt[, motif_pair := as.character(motif_pair)]
    dt
}))
tfam_dt[, motif_pair := factor(motif_pair, levels = unique(tfam_dt$motif_pair))]
tfam_dt[, variable := str_replace_all(variable, c("motif1" = "TF family 2", "motif2" = "TF family 1")), by = .I]
...
# heatmap of fraction of peaks that have a motif pair
mtd_gp <- copy(mtd_cta)
setnames(mtd_gp, "motif_pair", "motif_pair_old")
mtd_gp[, motif_pair_old := as.character(motif_pair_old)]
mtd_gp[, motif_pair := motif_pair_vec[as.character(motif_pair_old)]]
mtd_gp[, motif_pair := factor(motif_pair, levels = unique(tfam_dt$motif_pair))]
names_dt <- unique(mtd_gp[!is.na(gene_name_pair), .(motif_pair, gene_name_pair)])
hm_gp <- ggplot(mtd_gp, aes(
    x = cell_type, 
    y = motif_pair, 
    fill = frac_peak_pair
  )) +
  ...
  scale_y_discrete(
    labels = structure(
      sapply(unique(names_dt$gene_name_pair), function(x) {
        gs = str_split(as.character(x), " \\+ ")[[1]]
        paste0(
          ifelse(nchar(gs[1]) > 25, paste(substr(gs[1], 1, 23), "..."), gs[1]), 
          " + ",
          ifelse(nchar(gs[2]) > 25, paste(substr(gs[2], 1, 23), "..."), gs[2])
        )
      }),
      names = as.character(names_dt$motif_pair)
    )
  ) +
  ...
  labs(
    x = "cell type peaks",
    y = sprintf(
      "%s motif pairs present in > %.2f%% peaks", 
      length(unique(mtd_gp$motif_pair)),
      mta_pair_frac * 100
    ),
    fill = "fraction of \ncell type peaks\nwith both motifs"
  ) +
...
BBBBBBBBBBBBC
"
gp <- ct_gp + hm_gp + fm_gp + 
  plot_layout(design = layout, guides = "collect")

# save ggplot
ggsave(
  file.path(fig_dir, sprintf("motif-co-occurrence-%s-heatmap-%s.pdf", mta_pair_frac, ct)),
  gp, width = 16, height = 32
)

# save plotly heatmap
require(plotly)
require(htmlwidgets)
hm_gp <- ggplotly(hm_gp)
saveWidget(hm_gp, file.path(fig_dir, sprintf("motif-co-occurrence-%s-heatmap-%s.html", f_pairs, ct)))
```
