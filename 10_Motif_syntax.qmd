...
fwrite(
  mta_novl_dt, 
  file.path(syn_dir, sprintf("motif-hits-novl-%s-mona-q-%s.tsv.gz", arc_id, q)),
  sep = "\t"
)
```

Co-occurence of pairs of motifs

```{r}
count_co_occurrences <- function(dt) {
...
mta_novl_counts <- count_co_occurrences(mta_novl_dt)
saveRDS(
  mta_novl_counts, 
...
pks_ct <- pks_dt[broad_cell_type == ct]

# motif overlap in selected peaks
mta_novl_ct <- mta_novl_dt[peak %in% pks_ct$peak]
mta_novl_ct_counts <- count_co_occurrences(mta_novl_ct)

# save
saveRDS(
  mta_novl_ct_counts, 
  file.path(syn_dir, sprintf("motif-co-occurrences-%s-%s-mona-q-%s.rds", ct, arc_id, q))
)
```

Co-occurence enrichment: 
...
# fraction of peaks with hites to each motif
mta_novl_frac <- unique(mta_novl_ct[, .(motif, peak, motif_count)])
mta_novl_frac <- mta_novl_frac[, .(frac_peak = length(unique(.SD$peak))/n_peaks), by = motif][order(-frac_peak)]

# fraction of peaks with hits to both motifs
mta_pair_frac <- mta_novl_ct_counts / n_peaks

# enrichment
mta_novl_prod <- matrix(0, nrow = nrow(mta_pair_frac), ncol = ncol(mta_pair_frac))
rownames(mta_novl_prod) <- colnames(mta_novl_prod) <- colnames(mta_pair_frac)
for (i in rownames(mta_novl_prod)) {
  for (j in colnames(mta_novl_prod)) {
    mta_novl_prod[i, j] <- mta_pair_frac[i, j] / (mta_novl_frac[motif == i]$frac_peak * mta_novl_frac[motif == j]$frac_peak)
  }
}
...
saveRDS(
  mta_novl_prod, 
  file.path(syn_dir, sprintf("motif-co-occurrence-enrichment-%s-%s-mona-q-%s.rds", ct, arc_id, q))
)
```
...
```{r}
# get motif pairs from the lower triangle of the matrix
mta_novl_prod_lt <- mta_novl_prod
mta_novl_prod_lt[upper.tri(mta_novl_prod_lt)] <- NA
mta_novl_prod_dt <- as.data.table(mta_novl_prod_lt, keep.rownames = "motif1")
mta_novl_prod_dt <- melt(mta_novl_prod_dt, value.name = "enrichment", variable.name = "motif2", id.vars = "motif1")
mta_novl_prod_dt <- mta_novl_prod_dt[!is.na(enrichment)]

# add counts of hits to pair of motifs
mta_novl_count_dt <- as.data.table(mta_novl_ct_counts, keep.rownames = "motif1")
mta_novl_count_dt <- melt(mta_novl_count_dt, value.name = "count", variable.name = "motif2", id.vars = "motif1")
mta_enr <- merge.data.table(mta_novl_prod_dt, mta_novl_count_dt, by = c("motif1", "motif2"))

# add fractions of peaks with hits to both motifs
mta_novl_frac_dt <- as.data.table(mta_pair_frac, keep.rownames = "motif1")
mta_novl_frac_dt <- melt(mta_novl_frac_dt, value.name = "fraction", variable.name = "motif2", id.vars = "motif1")
mta_enr <- merge.data.table(mta_enr, mta_novl_frac_dt, by = c("motif1", "motif2"))

# for motifs with less than 2 hits, set enrichment to NA
mta_enr[count <= 2, enrichment := pmin(2, enrichment)]

# get top pairs
setorder(mta_enr, -enrichment)

# plot
mta_enr_gp <- mta_enr[enrichment > 1 & fraction > 0.1]
mta_enr_gp[, motif_pair := sprintf("%s + %s", motif1, motif2)]
gp_enr <- ggplot(mta_enr_gp, aes(count, enrichment)) +
  geom_point() +
  #geom_text_repel(
  #  data = mta_enr_gp[enrichment > 9 | count > 8000],
  #  aes(label = motif_pair), max.overlaps = Inf
  #) +
  labs(
...
mta_novl_frac <- mta_novl_ct[, .(frac_peak = length(unique(peak)) / n_peaks), by = motif]

# Fraction of peaks with hits to both motifs
mta_pair_frac <- mta_novl_ct_counts / n_peaks

# Create a lookup table for the motif fractions
motif_frac_lookup <- setNames(mta_novl_frac$frac_peak, mta_novl_frac$motif)[rownames(mta_pair_frac)]

# Compute the enrichment matrix
mta_novl_prod <- mta_pair_frac / (outer(motif_frac_lookup, motif_frac_lookup, FUN = "*"))
