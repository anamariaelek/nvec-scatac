...
res_dt_ <- res_dt[, .N, .(m1, m2)]
fwrite(
  res_dt_, 
  file.path(syn_dir, sprintf(
    "motif-co-occurrences-nonovl-%s-mona-q-%s.tsv.gz", 
    arc_id, q
  ))
)
```
...
for (ct in cts) {
  ...
  tryCatch({
    ...
    # Broad cell type peaks
    #pks_ct <- pks_dt[get(lvl) == ct]
...
Check enrichmenat of motif pairs accross different cell types.
...
enr_ct <- unique(enr_dt[, .(motif1, motif2, cell_type, enrichment)])
saveRDS(
  enr_ct, 
...
Combine with gene assignment data

```{r}
# load data
enr_dt <- readRDS(file.path(syn_dir, lvl, sprintf(
  "motif-co-occurrences-enrichment-%s-mona-q-%s.rds",
  arc_id, q
)))
