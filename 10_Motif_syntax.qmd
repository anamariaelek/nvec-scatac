...
# pks_dt <- pks_annt_dt[Log2FC>1.5][FDR<0.05]
...
mta_enr_dt <- mta_enr_dt[padj < 0.01 & fc > 1.5]
...
mta_pair_novl_enr <- mta_pair_novl_gen[
  motif1 %in% mta_enr_dt$motif | 
  motif2 %in% mta_enr_dt$motif
]
...
mdt_cnt <- merge.data.table(mta_pair_novl_sig, pks_num, by = "cell_type")
mdt_cnt[, frac_peak_pair := count_peak_pair / n_peaks, by = 1:nrow(mdt_cnt)]

# add motif pair column
mdt_cnt[, motif_pair := sprintf("%s + %s", motif1, motif2), by = 1:nrow(mdt_cnt)]
mdt_cnt[, gene_pair := sprintf("%s + %s", gene_motif1, gene_motif2), by = 1:nrow(mdt_cnt)]
mdt_cnt[, gene_name_pair := sprintf("%s + %s", gene_name_motif1, gene_name_motif2), by = 1:nrow(mdt_cnt)]
...
Inspect top pairs of enriched motifs per cell type.

```{r}
# top enriched motifs per cell type
ct <- "cnidocyte"
mdt_top <- mdt_cnt[cell_type==ct][order(-frac_peak_pair)]
mdt_oth <- mdt_cnt[cell_type!=ct][motif_pair %in% mdt_top$motif_pair]
mdt_cts <- rbindlist(list(mdt_top, mdt_oth))

# subset to cell types with sufficient peaks
mdt_cts <- mdt_cts[cell_type %in% cell_types_sub]

# fill in missing cell type
all_com <- CJ(
  motif_pair = unique(mdt_cts$motif_pair),
  cell_type = unique(mdt_cts$cell_type)
)
mdt_all <- merge(
  all_com, mdt_cts,
  by = c("cell_type", "motif_pair"),
  all.x = TRUE
)
mdt_all[is.na(count_peak_pair), count_peak_pair := 0]
mdt_all[is.na(frac_peak_pair), frac_peak_pair := 0]

# order motif pairs
mdt_all[, motif_pair := factor(motif_pair, levels = unique(mdt_top$motif_pair))]

# order cell types
stopifnot(all(mdt_all$cell_type %in% cell_types))
mdt_all[, cell_type := factor(cell_type, levels = unique(cell_types))]

# subset to plot
#n_pairs <- 200
#top_pairs <- unique(mdt_top$motif_pair)[1:n_pairs]
f_pairs <- 0.05
top_pairs <- unique(mdt_top[frac_peak_pair>f_pairs]$motif_pair)
mdt_gp <- mdt_all[motif_pair %in% top_pairs]

# cluster motif pairs
mdt_dc <- dcast.data.table(
  unique(mdt_gp[, .(motif_pair, cell_type, frac_peak_pair)]),
  motif_pair ~ cell_type, value.var = "frac_peak_pair"
)
mdt_mt <- as.matrix(mdt_dc[, -1])
rownames(mdt_mt) <- mdt_dc[[1]]
hclust_res <- stats::hclust(dist(mdt_mt, method = "manhattan"), method = "ward.D2")
motif_pair_lvl <- rownames(mdt_mt)[hclust_res$order]
mdt_gp[, motif_pair := factor(motif_pair, levels = motif_pair_lvl)]

# alternatively, order motif pairs by max value cell type
# mdt_gp[, max_ct := as.integer(.SD[order(-frac_peak_pair)][1]$cell_type), by = motif_pair]
# mdt_gp[, motif_pair := factor(motif_pair, levels = unique(mdt_gp[order(max_ct)]$motif_pair))]

# plot
names_dt <- unique(mdt_gp[!is.na(gene_name_pair), .(motif_pair, gene_name_pair)])
hm_gp <- ggplot(mdt_gp, aes(
    x = cell_type, 
    y = motif_pair, 
    fill = frac_peak_pair
  )) +
  geom_tile() +
  scale_y_discrete(
    breaks = names_dt$motif_pair,
    labels = sapply(unique(names_dt$gene_name_pair), function(x) {
      gs = str_split(x, " \\+ ")[[1]]
      paste0(
  ...
  labs(
    ...
    y = sprintf(
      ...
      length(unique(mdt_gp$motif_pair)),
      f_pairs * 100
    ),
    fill = "fraction of \ncell type peaks\nwith both motifs"
  ) +
...
ct_gp <- ggplot(pks_num, aes(x = cell_type, y = n_peaks, fill = cell_type)) +
  ...
  labs(
    y = "# peaks",
    title = sprintf("Motifs enriched in %s", ct)
  ) +
  theme(
...
# combine plots
require(patchwork)
gp <- ct_gp + hm_gp + plot_layout(ncol = 1, heights = c(1, 8))

# save
ggsave(
  file.path(fig_dir, sprintf("motif-co-occurrence-%s-heatmap-%s.pdf", f_pairs, ct)),
  gp, width = 8.5, height = 12
)
