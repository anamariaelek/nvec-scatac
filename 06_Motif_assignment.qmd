...
gold <- fread(file.path(ann_dir, "golden-marks-231124.tsv"), header = FALSE, col.names = c("common_name", "gene"), select = 1:2)
```
...
dapseq_dt <- dapseq_dt[success == "success"][common_name != "NvSox2"]
...
- motifs determined experimentally (CisBP, DAPseq)
- motifs infered by similarity regression
...
# gene - similarity regression infered motifs
infer_dt <- unique(fread(
  file.path(
    "results", "Archetypes",
    sprintf("motif-archetypes-%s.dict", arc_id)
  ), sep = "\t"
)[source == "CisBP_I", .(gene, archetype_name)])
setnames(infer_dt, c("gene", "motif_name"))
...
ups <- upset(
  as.data.frame(dr_eu),
  intersect = colnames(dr_eu),
  sort_intersections_by = "cardinality",
  width_ratio = 0.2
)

pdf(file.path(
  fig_dir, "motif-assignment-upset.pdf"
), width = 6, height = 4)
plot(ups)
dev.off()
```
...
Genes not expressed: 51
Genes not accessible: 24
Motifs not active: 1
...
Compare motifs assigned with different methods:

```{r}
mta_pwm <- readRDS(file.path(mta_dir, "motif-assignment-combined-pwms.rds"))
mta_acc_exp_dt <- fread(file.path(
  mta_dir, "motif-assignment-combined-activity-cell-type.tsv.gz"
))
mta_gen_sum <- unique(
  mta_acc_exp_dt[,.(gene,motif_name,motif_assignment)]
)[,motif_assingnments:=paste(sort(unique(.SD$motif_assignment)),collapse="+"),gene]
unique(mta_gen_sum[,.(gene,motif_assingnments)])[,.N,motif_assingnments][order(-N)]

method_col <- c(
  "archetype" = "#5ea3d3ff", 
  "orthology" = "#fb5e4cff", 
  "similarityreg" = "#bc80bd",
  "cisbp" = "#ffff6eff", 
  "dapseq" = "#4cfb62ff"
)

assgn <- "archetype+cisbp+dapseq+orthology"
genes <- mta_gen_sum[motif_assingnments==asgn, gene]
motfs <- mta_gen_sum[motif_assingnments==asgn, motif_name]
...
```{r}
genes <- unique(mta_gen_sum$gene)
motfs <- unique(mta_gen_sum$motif_name)
cat(sprintf("\n%s genes\n%s motifs", length(genes), length(motfs)))
...
for (gn in plot_genes) {
  ...
  pt_dt[, motif_assignment := factor(
    motif_assignment,
    levels = c("archetype", "orthology", "similarityreg", "cisbp", "dapseq")
  )]
  ...
  sim_mat <- compare_motifs(
    motifs = mta_pwm[unique(pt_dt$motif_name)],
    use.type = similarity,
    method = method,
  ...
  sim_mat_list[[gn]] <- sim_mat

}

# keep results in a data.table
sim_dt <- rbindlist(lapply(
  names(sim_mat_list), function(gn) {
    x <- sim_mat_list[[gn]]
    dt <- melt.data.table(
      as.data.table(x, keep.rownames = "motif1"),
      id.vars = "motif1",
      variable.name = "motif2",
      value.name = "motif_similarity"
    )
    dt <- merge.data.table(
      dt,
      unique(mta_gen_sum[gene == gn, .(motif1 = motif_name, motif1_assignment = motif_assignment)]),
      all.x = TRUE,
      by = "motif1",
      sort = FALSE,
    ...
    dt <- merge.data.table(
      dt,
      unique(mta_gen_sum[gene == gn, .(motif2 = motif_name, motif2_assignment = motif_assignment)]),
      all.x = TRUE,
      by = "motif2",
      sort = FALSE,
      allow.cartesian = FALSE
...
# get all assigned motif pairs per gene in a data table
setcolorder(sim_dt, c("motif1", "motif1_assignment", "motif2", "motif2_assignment"))
fwrite(
  sim_dt,
  file.path(mta_dir, "motif-assignment-combined-similarity.tsv.gz"),
  sep = "\t", compress = "gzip"
)

# parse pairs of assigned motifs
sim_pairs_dt <- copy(sim_dt)[motif1 != motif2]
sim_pairs_dt[, motif_pair := paste0(sort(c(.SD$motif1, .SD$motif2)), collapse = "+"), .I]
sim_pairs_dt[, motif_assignment_pair := paste0(sort(c(.SD$motif1_assignment, .SD$motif2_assignment)), collapse = "+"), .I]
sim_pairs_dt <- unique(sim_pairs_dt[, .(gene, motif_pair, motif_assignment_pair, motif_similarity)])

# order
cats <- sim_pairs_dt[, median(motif_similarity), motif_assignment_pair][order(-V1)]$motif_assignment_pair
sim_pairs_dt[, motif_assignment_pair := factor(motif_assignment_pair, levels = cats)]

# plot distribution of similarity values
gp_sim_dens <- ggplot(sim_pairs_dt, aes(motif_similarity, motif_assignment_pair)) +
  geom_boxplot(fill = "#bababa")
gp_sim_nums <- ggplot(sim_pairs_dt, aes(motif_assignment_pair)) +
  geom_bar(fill = "#bababa", color = "#000000") +
  coord_flip() +
  scale_y_continuous(trans = "log10") +
  geom_text(stat = "count", aes(label = ..count..), hjust = 1) +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title.y = element_blank())
require(patchwork)
gp_sim <- gp_sim_dens + gp_sim_nums + plot_layout(ncol = 2)
ggsave(
  file.path(fig_dir, "tf-motfis-similarity-box.pdf"),
  gp_sim,
  width = 14, height = 6
)
```

* plot motif similarities.

```{r}
require(ComplexHeatmap)

for (assgn in c(
  # assignment vs experiment
  "archetype+cisbp+dapseq+orthology",
  "archetype+cisbp+orthology",
  "archetype+dapseq+orthology",
  "archetype+dapseq+similarityreg"
  # different assignments
#  "archetype+similarityreg",
#  "orthology+similarityreg"
)) {
  
  # gene
  genes <- unique(mta_gen_sum[motif_assingnments==assgn, gene])
  motfs <- unique(mta_gen_sum[motif_assingnments==assgn, motif_name])
  cat(sprintf("%s: %s genes", assgn, length(genes)))
  
  # where to save the plot
  pdf(
    file.path(fig_dir, sprintf("tf-motfis-similarity-%s.pdf", assgn)), 
    height = 10, width = 12
  )
  ...
  for (gn in genes) {
    ...
    # similarities 
    sim_mat <- sim_mat_list[[gn]]
       
    # annotation
    ha_col = HeatmapAnnotation(
      method = pt_dt[match(rownames(sim_mat), motif_name)]$motif_assignment,
    ...
    mlogs <- view_motifs(mta_pwm[rownames(sim_mat)])
    print(mlogs)
    
    # short names
    sim_mat_copy <- sim_mat
    dap_ids <- grep("homer|streme|dimont", rownames(sim_mat_copy))
    dap_nms <- str_extract(rownames(sim_mat_copy)[dap_ids], "\\d+_(homer|streme|dimont)")
    rownames(sim_mat_copy)[dap_ids] <- dap_nms
    colnames(sim_mat_copy)[dap_ids] <- dap_nms
    
    # plot
    col_fun = colorRamp2(c(0, 0.25, 0.5, 0.75, 1), c("#4d4d4d", "#999999", "#e0e0e0", "#f4a582", "#b2182b"))
    hm <- Heatmap(
      sim_mat_copy,
      name = sprintf("%s\nmotif similarity", method),
      col = col_fun,
      show_row_names = TRUE,
...
* plot pairs of expression-activity scaterplots
...
#  "orthology+similarityreg"
  ...
  pdf(
    file.path(fig_dir, sprintf("tf-expression-activity-scatter-plots-%s.pdf", assgn)), 
    height = 20, width = 16
  )
  ...
  for (gn in plot_genes) {
    ...
    mt_list <- lapply(seq_len(nplots), function(i) {
      ...
      logo_ttl <- paste(
        mt_mod, 
        ifelse(grepl("homer|streme|dimont", mm), str_extract(mm, "homer|streme|dimont"), mm),
        sep = ": "
      )
...
# exclude experimental/orthology motifs for some genes
blck_gns <- c(
  "Nvec_vc1.1_XM_001639701.3",
  "Nvec_vc1.1_XM_048728976.1",
  "Nvec_vc1.1_XM_032376930.1",
  "Nvec_vc1.1_XM_032381562.2",
  "Nvec_vc1.1_XM_032381586.2",
  "Nvec_vc1.1_XM_001639307"
)
if (any(blck_gns %in% genes_dt$gene)) {
  genes_dt <- genes_dt[!gene %in% blck_gns]
}
...
# load original archetype assignments
arc_id <- "PPM-PCC-0.8-IC0.5-5bp"
arc_gen_dt <- fread(file.path(
  "results", "Archetypes",
  sprintf("motif-assignment-archetypes-%s.tsv.gz", arc_id)
))
...
Plot motifs PWMs logos, grouped by class.

```{r}
# chromvar scores
chromvar_mta <- readRDS(file.path(mta_dir, "ChromVAR", "MotifsSEACellsMatrix", "Matrix-Motifs-SEACells.rds"))
chromvar_mta_max <- apply(as.matrix(chromvar_mta), 1, max)
chromvar_arc <- readRDS(file.path(arc_dir, sprintf("ChromVAR-%s", arc_id), "ArchetypesSEACellsMatrix", "Matrix-Archetypes-SEACells.rds"))
chromvar_arc_max <- apply(as.matrix(chromvar_arc), 1, max)

# plot logos
for (cl in mta_gen_dt[,.N,class][order(-N)]$class) {
  
  mta_cls_dt <- unique(mta_gen_dt[class==cl])[order(archetype_family)]
  
  # get motifs
  mta_cl_pwm <- sapply(mta_cls_dt$gene, function(g) {
    # assigned motif
    n <- mta_cls_dt[gene == g]$archetype_name
    m <- mta_pwm[[n]]
    # gene name
    gn <- mta_cls_dt[gene == g]$gene_name
    if (length(gn) == 0 | gn == "") gn <- str_trunc(mta_cls_dt[gene == g]$og, 35)
    # archetype family
    f <- mta_cls_dt[gene == g]$archetype_family
    # assigned motif correlations
    sc <- mta_gen_dt[gene==g]$gene_score_correlation
    ec <- mta_gen_dt[gene==g]$gene_expression_correlation
    zs <- chromvar_mta_max[n]
    if (is.na(zs)) zs <- chromvar_arc_max[n]
    cs <- sprintf("gene score cor: %.2f; expression cor: %.2f; max zscore: %.2f", sc, ec, zs)
    # original archetype assignment
    a <- arc_gen_dt[gene==g]$archetype_name
    so <- arc_gen_dt[gene==g]$gene_score_correlation
    eo <- arc_gen_dt[gene==g]$gene_expression_correlation
    zo <- chromvar_arc_max[a]
    if (a != n & (so > sc | eo > ec | zo > zs)) {
      co <- sprintf("%s; gene score cor: %.2f; expression cor: %.2f; max zscore: %.2f", a, so, eo, zo)
    } else {
      co <- ""
    }
    # update moif name
  ...
  if (cl == "archetype") {
    pdf(
      file.path(fig_dir, sprintf("motifs-genes-logos-%s.pdf", cl)),
      width = 10, height = 4
    )
    for (m in mta_cl_pwm) {
      print(view_motifs(list(m)) + labs(title = m@name))
    }
    dev.off()
  } else {
    pdf(
      file.path(fig_dir, sprintf("motifs-genes-logos-%s.pdf", cl)),
      width = 10, height = pmax(length(unique(mta_cls_dt$gene)) * 1.8, 5)
    )
    print(view_motifs(mta_cl_pwm))
    dev.off()
  }
...
# DAPseq data
dap_dt <- fread(file.path(dap_dir, "dapseq-results-summary-v6.tsv.gz"))
```
...
Shortlist candidates for experimental validation.

```{r}
# motifs assignment info
dr_dt <- fread(file.path(mta_dir, "motif-assignment-combined.tsv.gz"), sep = "\t")
dr_dt <- dr_dt[gene %in% tfan$gene]

# motif activity info
# # # # I NEED TO UPDATE THIS TO INCLUDE ALL MOTIFS # # # #
#acc_exp_ct <- fread(file.path(
#  mta_dir, "gene-expression-accessibility-archetype-motif-deviation-cell-type.tsv.gz"
#))
#setnames(acc_exp_ct, "archetype_name", "motif_name")

# assigned motifs similarities
sim_dt <- fread(file.path(mta_dir, "motif-assignment-combined-similarity.tsv.gz"))


# TF importane from GRNs
grn_dt <- fread(file.path(
  "results", "GRN", "networks", "cell_type",
  "grn_tfs_info_cell_type_expression_fc_0.4_accessibility_access_0.4_chromvar_4.tsv.gz"
))
grn_gen_dt <- unique(grn_dt[, .(cell_type, stage, gene, expression_fc, expression_umifrac, gene_score)])
grn_mot_dt <- unique(grn_dt[, .(cell_type, stage, gene, motif_name = motif, zscore)])

# all TFs info
tf_dt <- tfan[gene %in% rownames(exp_mt) & gene %in% rownames(acc_mt)]

# add all TFs info to motif assignment table
...
mot_dt <- merge.data.table(
  mot_dt, grn_mot_dt,
  by = c("gene", "motif_name", "cell_type", "stage"),
  all.x = TRUE, sort = FALSE
)

# order
stopifnot(all(mot_dt[!is.na(cell_type)]$cell_type %in% cell_types))
mot_dt[, cell_type := factor(cell_type, levels = cell_types)]
mot_dt[, stage := factor(stage, levels = c("gastrula", "adult"))]

# prioritize genes without dapseq
mot_dt[, has_dapseq := gene %in% dap_dt$gene]
mot_dt[is.na(has_dapseq), has_dapseq := FALSE]
mot_dt[, has_dapseq := factor(has_dapseq, levels = c(TRUE, FALSE))]
mot_dt[, has_dapseq_success := gene %in% dap_dt[success == "success"]$gene]
mot_dt[is.na(has_dapseq_success), has_dapseq_success := FALSE]
mot_dt[, has_dapseq_success := factor(has_dapseq_success, levels = c(TRUE, FALSE))]

# prioritize genes 
# 1) high expression in gastrula cell types
setorder(mot_dt, stage, cell_type, -has_dapseq, -expression_fc, -gene_score, zscore, na.last = TRUE)
genes_1 <- unique(mot_dt[expression_fc > 2 & gene_score > 2 & zscore > 4 & stage == "gastrula" & has_dapseq == FALSE]$gene)
# 2) high expression in neuronal cell types
genes_2 <- unique(mot_dt[expression_fc > 2 & gene_score > 2 & zscore > 4 & grepl("neuron|cnidocyte", cell_type) & has_dapseq == FALSE]$gene)
# 3) high zscore in any cell type
genes_3 <- unique(mot_dt[expression_fc > 2 & gene_score > 2 & zscore > 4 & has_dapseq == FALSE]$gene)
length(genes_1)
# 30
length(setdiff(genes_2, genes_1))
# 9
length(setdiff(genes_3, c(genes_1, genes_2)))
# 15
gene_levels <- unique(c(genes_1, genes_2, genes_3))
gene_levels <- c(gene_levels, setdiff(unique(mot_dt$gene), gene_levels))
gene_levels <- c(
  setdiff(gene_levels, mot_dt[has_dapseq == TRUE]$gene),
  unique(mot_dt[has_dapseq == TRUE]$gene)
)
stopifnot(all(mot_dt$gene %in% gene_levels))
mot_dt[, gene := factor(gene, levels = unique(gene_levels))]
setorder(mot_dt, gene, stage, cell_type, -expression_fc, -gene_score, zscore, na.last = TRUE)
mot_dt[, rank := .GRP, .(gene)]

# select top cell type per gene
top_dt <- unique(mot_dt[, .SD[1], .(gene, motif_name)])
setcolorder(top_dt, c(
  "rank", "gene", "gene_name", "common_name", "og", "pfam",
  "motif_assignment", "motif_name", 
  "cell_type", "stage", 
  "expression_fc", "expression_umifrac", "gene_score", "zscore"
))

# keep track of the assigned motif
top_dt[, motif_assigned := unique(.SD[!is.na(zscore)]$motif_assignment), .(gene, cell_type)]

# combine all similarity regression assigned motifs
top_dt[motif_assignment == "similarityreg", motif_name := paste(unique(sort(.SD$motif_name)), collapse = ","), .(gene)]
top_dt <- unique(top_dt)

# combine all dapseq motifs
top_dt[motif_assignment == "dapseq", motif_name := paste(unique(sort(.SD$motif_name)), collapse = ","), .(gene)]
top_dt <- unique(top_dt)

# combine all dapseq motifs
top_dt[motif_assignment == "orthology", motif_name := paste(unique(sort(.SD$motif_name)), collapse = ","), .(gene)]
top_dt <- unique(top_dt)

# keep assigned zscore for each gene
top_dt[, zscore := max(.SD$zscore, na.rm = TRUE), gene]
top_dt[is.infinite(zscore), zscore := NA]
top_dt <- unique(top_dt)

# move motifs to columns
top_dt[, motif_assignment := paste("motif_", motif_assignment, sep = "")]
top_dc <- dcast.data.table(
  top_dt,
  rank + gene ~ motif_assignment,
  value.var = "motif_name"
)
#setnames(
#  top_dc,
#  c("archetype", "dapseq", "cisbp", "similarityreg", "orthology"),
#  c("motif_archetype", "motif_dapseq", "motif_cisbp", "motif_similarityreg", "motif_orthology")
#)

res_dt <- merge.data.table(
  top_dc, unique(top_dt[, .(
    rank, gene, gene_name, common_name, og, pfam, cell_type, stage,
    expression_fc, expression_umifrac, gene_score, zscore,
    has_dapseq, has_dapseq_success, motif_assigned
  )]),
  by = c("rank", "gene"),
  all.x = TRUE, sort = FALSE
)
...
setcolorder(res_dt, c(
  "rank", "gene", "gene_name", "common_name", "og", "pfam",
  "motif_assigned", "has_dapseq", "has_dapseq_success",
  "motif_dapseq", "motif_cisbp", "motif_archetype", "motif_orthology", "motif_similarityreg",
  "cell_type", "stage", "expression_fc", "expression_umifrac", "gene_score", "zscore"
))

# save
fwrite(
  res_dt,
  file.path(mta_dir, "motif-assignment-combined-genes-candidates-info.tsv.gz"),
  sep = "\t", compress = "gzip"
)
```
