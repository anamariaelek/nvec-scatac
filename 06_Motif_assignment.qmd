...
mot_dt[, has_dapseq := "dapseq" %in% .SD$motif_assignment, .(gene)]
...
genes_1 <- unique(mot_dt[expression_fc > 2 & gene_score > 2 & zscore > 4 & stage == "gastrula"]$gene)
# 2) high expression in neuronal cell types
genes_2 <- unique(mot_dt[expression_fc > 2 & gene_score > 2 & zscore > 4 & grepl("neuron|cnidocyte", cell_type)]$gene)
# 3) high zscore in any cell type
genes_3 <- unique(mot_dt[expression_fc > 2 & gene_score > 2 & zscore > 4]$gene)
length(genes_1)
# 38
...
top_dt <- mot_dt[, .SD[1], .(gene, motif_name)]
...
mot_dt[, motif_assigned := unique(.SD[is.na(zscore)]$motif_assignment), .(gene)]
