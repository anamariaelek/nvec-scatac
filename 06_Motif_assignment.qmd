...
# motif PWMs
cisbp_mt <- readRDS(file.path(cisbp_dir, "cisbp_nvec_motifs.rds"))
names(cisbp_mt) <- sapply(cisbp_mt, function(m) m@name)
cisbp_mt <- cisbp_mt[cisbp_dt$Motif_ID]

# motifs vector
motifs_cisbp <- cisbp_mt
names(motifs_cisbp) <- cisbp_dt$TF_Common_Name
```

- motifs we retrieved from DAPseq: FoxQ2d
...
# motifs vector
motifs_dapseq <- c(
  "FoxQ2d" = mta_read_homer_mod(file.path(dap_dir, "FoxQ2d.motif1.motif"))
)
```
...
```{r}
motifs <- c(motifs_dapseq, motifs_cisbp)
...
# table with mapping: gene - selected archetype - direct motif
genes <- c(genes_dapseq, genes_cisbp)
genes <- genes[genes %in% rownames(exp_mt) & genes %in% rownames(acc_mt)]
mta_gen_vc <- structure(names(genes), names = genes)
mta_gen_dr <- mta_gen_dt[gene %in% genes]
dr_dt <- data.table(
  "gene" = mta_gen_dr$gene,
  "archetype" = mta_gen_dr$archetype_name
)[, direct_motif := mta_gen_vc[gene]]
dr_dt <- melt.data.table(dr_dt, id.vars = "gene", variable.name = "motif_assignment", value.name = "motif_name")
...
for (gn in unique(mta_acc_exp_dt$gene)) {
  ...
  if (length(mt) == 1) {
    ...
    # patchwork
    patch <- 
      mt_list[[1]][["motif"]] + mt_list[[2]][["motif"]] +
      mt_list[[1]][["scatterplot"]] + mt_list[[2]][["scatterplot"]] + 
      plot_layout(ncol = 2, heights = c(1, 5)) +
...
arch_list <- universalmotif::read_meme(arch_fn)
arch_nms <- sapply(arch_list, function(m) str_extract(m@name, "ARCH\\d+"))
names(arch_list) <- arch_nms

# save all motifs
all_list <- c(arch_list, mots_mta_lst)
write_meme(all_list, file.path(mta_dir, "motif-archetypes-all.meme"), overwrite = TRUE)
...
if (length(genes_assigned) > 0) {
  ...
  mta_gen_dt[match(genes_assigned, gene), ':='(
    ...
    source = "experiment",
    rank = 0,
    #gene_score_correlation = mot_gen_cor[.SD$gene, genes_names[.SD$gene]],
...
saveRDS(
  ...
  file.path(mta_dir, sprintf(
    "motif-archetypes-all-similarity-%s-%s.rds", 
    similarity, method
  ))
)
...
min_sim <- 0
k <- 1000
while (min_sim < 0.65 & k < nrow(sim_mat)) {
  k <- k + 1
  message(k)
  ctr <- cutree(hc, k = k)
  ctr_sim <- sapply(unique(ctr), function(cl) {
    ms <- names(which(ctr == cl))
    mean(sim_mat[ms, ms], na.rm = TRUE)
  })
  min_sim <- min(ctr_sim)
}
# 0.65: 1148
...
length(unique(assign_dt$gene)) # 548
...
arc_plt_dt[gene_label == "" & gene != "" & common_name == "" & og != "", gene_label := ifelse(nchar(og)>25, paste0(substr(og, 1, 22), "..."), og)]
...
Plot per TF scatterplots of expression vs activity (deviation Z score) for selected genes (to show in the figure).
...
gp_acc_exp_ann <- gp_ctbar_v + gp_dotmap + gp_ctbar_h +
  plot_layout(design = design, widths = c(1, 200), heights = c(200, 1))

# save
ggsave(
  file.path(fig_dir, sprintf("motif-activity-gene-expression-FC-%s-zscore-%s.pdf", fc_thr, zs_thr)),
  gp_acc_exp_ann,
  height = 10, width = 9, limitsize = FALSE
)
```
