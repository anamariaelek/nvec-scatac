...
method_col <- c("archetype" = "#5ea3d3ff", "orthology" = "#fb5e4cff", "cisbp" = "#ffff6eff", "dapseq" = "#4cfb62ff")
...
for (assgn in c(
  ...
  for (gn in plot_genes) {
    ...
    # motif logos
    # dir.create(file.path(fig_dir, "tmp"), showWarnings = FALSE)
    # motif_imgs <- c()
    #for (mm in rownames(sim_mat)) {
    #  mot <- ggseqlogo::ggseqlogo(mta_pwm[[mm]]@motif) + theme_void()
    #  mot_fn <- file.path(fig_dir, "tmp", paste0(mm, ".png"))
    #  mot_fn <- str_replace_all(mot_fn, "\\/", "_")
    #  ggsave(mot_fn, height = 3, width = 5)
    #  motif_imgs <- c(motif_imgs, mot_fn)
    #}
    #ha_mot = HeatmapAnnotation(
    #  motif = anno_image(motif_imgs),
    #  which = "row", 
    ...
    hm <- Heatmap(
      ...
      #right_annotation = ha_mot,
      cluster_rows = FALSE,
      cluster_columns = FALSE,
      border = FALSE,
      width = unit(5, "in"), #ncol(mat)*unit(5, "mm"), 
      height = unit(5, "in") #nrow(mat)*unit(5, "mm")),
    )
    draw(hm)
  ...
  motif_sims <- rbindlist(lapply(
    names(sim_mat_list), function(gn) {
      ...
      dt <- merge.data.table(
        dt,
        unique(mta_gen_sum[, .(motif1 = motif_name, motif1_assignment = motif_assignment)]),
        all.x = TRUE,
        by = "motif1",
        sort = FALSE
      )
      dt <- merge.data.table(
        dt,
        unique(mta_gen_sum[, .(motif2 = motif_name, motif2_assignment = motif_assignment)]),
        all.x = TRUE,
        by = "motif2",
        sort = FALSE
      )
...
sim_dt <- sim_dt[motif1 != motif2]
sim_dt[, motif_pair := paste0(sort(c(.SD$motif1, .SD$motif2)), collapse = "+"), .I]
sim_dt[, motif_assignment_pair := paste0(sort(c(.SD$motif1_assignment, .SD$motif2_assignment)), collapse = "+"), .I]
sim_dt <- unique(sim_dt[, .(gene, motif_pair, motif_assignment_pair, motif_similarity)])

# order
cats <- sim_dt[, median(motif_similarity), motif_assignment_pair][order(-V1)]$motif_assignment_pair
sim_dt[, motif_assignment_pair := factor(motif_assignment_pair, levels = cats)]

# plot distribution of similarity values
gp_sim_dens <- ggplot(sim_dt, aes(motif_similarity, motif_assignment_pair)) +
  geom_boxplot(fill = "#bababa")
gp_sim_nums <- ggplot(sim_dt, aes(motif_assignment_pair)) +
