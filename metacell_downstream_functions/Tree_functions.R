...
#' @param vargenes vector, list of variable genes to use in tree building process. Default is NULL, which means that all genes are used.
#' @param p numeric, fraction of variable genes to use for tree building (default is 0.75)
#' @param method character, tree building method to use (default is `hclust`). Possible methods: `hclust`, `nj`
#' @param cor_method character, `cor` method (default is `pearson`) to obtain distance matrix from gene expression data
#' @param hclust_method character, `hclust` method (default is `average`); only used in `method` is `hclust`
#' 
#' @return object of class `phylo` where tips are columns in the `x` expression matrix
#' 
sct_tree_downsampling = function(x, vargenes=NULL, p=0.75, method="hclust", cor_method = "pearson", hclust_method = "average", pml_model = "JC") {
	...
	# get distance matrix
	dmat = as.dist( 1 - cor(x[sample(vargenes, p * length(vargenes)),], method=cor_method) )

	# get phylogeny
	if (method == "hclust") {
		# hclust
		phy = as.phylo(hclust(dmat, hclust_method)) 
	} else if (method == "nj") {
		# neighbour-joining in ape
		phy = as.phylo(ape::nj(dmat)) 
	} else {
		stop("`method` parameter should be either `hclust` or `nj`")
	}
...
#' Build trees and perform ancestral reconstruction
#' 
#' @param x matrix, genes in rows, cell types (or metacells) in columns
#' @param vargenes vector, list of variable genes to use in tree building process. Default is NULL, which means that all genes are used.
#' @param p numeric, fraction of variable genes to use for tree building (default is 0.75)
#' @param method character, tree building method to use (default is `hclust`). Possible methods: `hclust`, `nj`, `ml`, `parsimony`. 
#'     `ml` and `parsimony` require `x` to be a categorical character matrix (e.g. where 0/1 indicate on/off gene activity).
#' @param cor_method character, `cor` method (default is `pearson`) to obtain distance matrix from gene expression data
#' @param hclust_method character, `hclust` method (default is `average`); only used in `method` is `hclust`
#' @param pml_model character, `pml` evolutionary model (default is `JC`); only used in `method` is `ml`
#' @param ancestral logical, whether to perform ancestral trait reconstruction using parsimony
#' @param ancestral_method character, method to use for ancestral reconstruction methods (`ancestral.pars`), defaults to `MPR`.
#' 
#' @return list with three objects: `phy` is an object of class `phylo` where tips are columns in the `x` expression matrix,
#'     `anc` is a `phyDat` object containing the parsimony reconstruction of on/off genes at each tip in the tree, and
#'     `variable_genes` is a character vector with the gene order in the `anc` object (rows for each node)
#' 
sct_tree_reconstruction = function(
	x, 
	vargenes=NULL, 
	p=0.75, 
	method="hclust", 
	...
	pml_model = "JC",
	ancestral=TRUE,
	ancestral_method = "MPR") {
	
	# if no variable genes are given, use all
	if (is.null(vargenes)) {
		vargenes = rownames(x)
	}
	
	# get distance matrix and phydat object
	# used for hclust or nj
	xmat = x[ sample(vargenes, p * length(vargenes)), ]
	dmat = as.dist( 1 - cor(xmat, method=cor_method) )
	# phydat alignment object, used for parsimony and ml methods AND ancestral reconstructions
	pmat = t(unique(xmat))
	pmat_genes = colnames(pmat)
	pmat_states = sort(unique(as.vector(pmat)))
	pdat = phangorn::as.phyDat(pmat, type="USER", levels = pmat_states, allLevels = TRUE, names = pmat_genes)
	...
	if (method == "hclust") {
		...
		# hamming distance from alignment
		pdis = phangorn::dist.hamming(pdat)
		# init tree search with a NJ tree
		phy_nj = phangorn::NJ(pdis)
		# compute ml tree, starting from NJ tree and alignment
		phy_fit = phangorn::pml(phy_nj, data=pdat)
		phy = phangorn::optim.pml(phy_fit, model = pml_model, rearrangement = "stochastic")
		...
		# init tree search with a NJ tree
		phy_nj = phangorn::NJ(pdis)
		# compute parsimony tree, starting from NJ tree and alignment
		phy = phangorn::optim.parsimony(phy_nj, data=pdat)
		
	} else {

		stop("`method` parameter should be either `hclust`, `nj`, `parsimony`, `ml`")

	}

	# do ancestral reconstruction, based on computed tree
	# (valid for parsimony and ml, untested for other methods but it should work anyway)
	if (ancestral) {
		anc = phangorn::ancestral.pars(phy, data=pdat, return = "phyDat")
		anc = lapply(anc, FUN = function(n) pmat_states [ n ] )
		variable_genes = pmat_genes
	} else {
		anc = NULL
		variable_genes = NULL
	}
	...
	# return
	return(list(phy = phy, anc = anc, variable_genes = variable_genes))
...
# Compare two trees
sct_compare_trees = function(tree1_fn, tree2_fn, outfile_prefix=NULL, labelcols=NULL) {
	# load trees
...
#' 
sct_tree_from_ensemble_clustering = function(
	...
	if (clustering_algorithm == "hclust") {
		for (i in 1:n) { 
			...
			hc=hclust(as.dist(1 - cor( x[ids,],method=cor_method )), clustering_method)
		...
		for (i in 1:n) {
			if (bootstrap) p=1
			ids=sample(vargenes,p * length(vargenes),replace=bootstrap)
			hc=as.hclust(force.ultrametric(root(nj(as.dist(1 - cor( x[ids,],method=cor_method ))),outgroup=1),method="extend"))
			ensemble_hc[[i]]=cutree(hc,h=hs[i])
		}
	}
...
#' 
sct_list_trees_as_multiPhylo = function(x) {
	mp = x[[1]]
	for (i in 2:length(x))
		mp = c(mp, x[[i]])
	return(mp)
...
#' 
sct_di2multi_support <- function(tree, support, thrs) {
...
#' Get node supporting genes for internal nodes in a tree
#' 
#' @param tree corresponding to feature matrix
#' @param feature_matrix, rows should be features (genes or motifs) and columns should be 
#'   tips of the tree; data should be metacell log fold enrichment over the median ("footprint" mc_fp)
...
#' 
#' @return a list of node supporting features:
#'   top_features = for every internal node, top features in all the tips that are below this node, 
#'   top_feature_anti = for every internal node, top_features in all the tips that are NOT below this node
#'   top_feature_sister = for every internal node, top_features in all the tips that are below it's closest internal node
#'   splits_mat = matrix storing the binary split info, i.e. which tips are below each node
#'   heatmap = heatmap of splits_mat
						       
sct_tree_gap_genes <- function(
	...
	for (node in nodes) {
		if (node %in% calc_nodes) {
			if (verbose) print(sprintf("Starting analysis for node %s", node))
			#i=i+1
			...
			if (verbose) print(sprintf("Tips in: %s", paste(tips_in,collapse=", ")))
			...
			if (verbose) print(sprintf("Sister tips: %s", paste(tips_sister,collapse=", ")))
			tips_out <- setdiff(treet@phylo$tip.label,tips_in)
			if (verbose) print(sprintf("Out tips: %s", paste(tips_out,collapse=", ")))
			...
			if (method == "median") {
				fin=apply(feature_matrix[,tips_in,drop=FALSE],1,median) > feature_in_thrs
				fin_inv=apply(feature_matrix[,tips_out,drop=FALSE],1,median) > feature_in_thrs
			} else if (method == "absolute") {
				fin=apply(feature_matrix[,tips_in,drop=FALSE], 1, function(x) 
					#all(x > feature_in_thrs)
					!(sum(x > feature_in_thrs) < .calc_leakyness(abs_leakyness,n=length(tips_in)))
				)
				fin_inv=apply(feature_matrix[ ,tips_out,drop=FALSE ], 1, function(x) 
					#all(x > feature_in_thrs)
					!(sum(x > feature_in_thrs) <  .calc_leakyness(abs_leakyness,n=length(tips_out)))
				)
			}
			if (methodbg == "median") {
				fout=apply(feature_matrix[,tips_out,drop=FALSE],1,median) < feature_out_thrs
				fout_inv=apply(feature_matrix[,tips_in,drop=FALSE],1,median) < feature_out_thrs
				fout_sister=apply(feature_matrix[,tips_sister,drop=FALSE],1,median) < feature_out_thrs
			} else if (methodbg == "absolute") {
				fout=apply(feature_matrix[,tips_out,drop=FALSE], 1, function(x) 
					#all(x < feature_out_thrs)
					!(sum(x < feature_out_thrs) < .calc_leakyness(abs_leakynessbg,n=length(tips_out)))
				)
				fout_inv=apply(feature_matrix[,tips_in,drop=FALSE], 1, function(x) 
					#all(x < feature_out_thrs)
					!(sum(x < feature_out_thrs) < .calc_leakyness(abs_leakynessbg,n=length(tips_in)))
				)
				fout_sister=apply(feature_matrix[,tips_sister,drop=FALSE], 1, function(x) 
					#all(x < feature_out_thrs)
			...
			if (length(f_in) > 0){
				...
				if (length(f_in) > 1)
					top_feat=top_feat[order(apply(feature_matrix[top_feat,tips_in,drop=FALSE],1,median),decreasing=TRUE)] 
				top_features[[node]]=top_feat
			} else { 
				top_features[[node]]="NONE"
			}	
			...
			if (length(f_sister) > 0) {
				...
				top_feature_sister[[node]]="NONE"
			}	
			
			if (length(f_out) > 0){
				top_anti=rownames(feature_matrix)[f_out]
				if (length(f_out) > 1)
					top_anti=top_anti[order(apply(feature_matrix[top_anti,tips_out,drop=FALSE],1,median),decreasing=TRUE)]
				top_feature_anti[[node]]=top_anti
			} else { 
				top_feature_anti[[node]]="NONE"
			}
			
			if (verbose) print(sprintf("Finished analysis for node %s", node))
...
sct_save_tree_files <- function(
	...
	# extract method from tree object
	method <- str_extract(tree_rds,"nj|hclust(?=\\.RDS)")
	if (is.na(method)) { 
		method="hclust"
	}
	...
	p <- 0.5
	for (i in 1:iter) { 
		support_trees[[i]] = sct_tree_downsampling(mc_fp,vargenes=var_genes,p=p,method=method) 
	}
	...
	ape::plot.phylo(tree,tip.color=tip_color,cex=1.5)
	...
	polit <- 0.1
	supp <- polit * iter
	polytree <- sct_di2multi_support(tree, support, supp)
	...
	par(mar=rep(2, 4))
	ape::plot.phylo(polytree, main=sprintf("Polytomies: support < %.0f%%",polit * 100), cex=1.5, tip.color=tip_color)
	dev.off()
...
#'   
sct_plot_tree_matrix <- function(
	mat, tree, build_tree=TRUE, rev=TRUE,
	ctcol=NULL, hmapcol=NULL, name, 
	...
	if (build_tree) {
		gt <- ggplot_build( ggtree(tree) + theme_tree2() + geom_tiplab() )
		gtd <- gt$data[[3]]
		setDT(gtd)
		setorder(gtd,"y")
	...
	if (!is.null(ctcol)) {
		...
		names_col = gpar(col = ctcol[labels_ord])
	} else {
		row_ha = NULL
		col_ha = NULL
		names_col = gpar(col = "black")
	}
	...
	hm <- ComplexHeatmap::Heatmap(
		...
		row_names_gp = names_col,
		column_names_gp = names_col,
...
#' 
sct_tree_color_tips <- function(tree=NULL, tree_file=NULL, out_file=NULL, ctcol) {
	
	# load tree or retrieve it from `tree` parameter
	if (is.null(tree)) {
		...
		if (all.equal(tree_file,out_file)) {
