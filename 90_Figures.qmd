...
for (gs in c("gastrula", "adult", "both")) {
  for (ps in c("gastrula", "adult", "both")) {
    ...
    ct <- sprintf("peaks_%s_genes_%s", ps, gs)
    pks_dt <- dynm_acc_dt[gene_stage == gs & peak_stage == ps & promoter == "NO"]
    pks <- unique(pks_dt$peak)
    print(sprintf("%s peaks open in %s and assocaited to gene open in %s", length(pks), ps, gs))
...
col_fun <- colorRamp2(
  c(min(mat_fc, na.rm = TRUE), 0, max(mat_fc, na.rm = TRUE)),
...
hm_enr <- Heatmap(
  ...
  cell_fun = function(j, i, x, y, width, height, fill) {
    # Draw dots
    scaled_size = pmin(mat_padj / 3, 1)  # cap
    grid.circle(
      x = x, y = y,
      r = unit(scaled_size[i, j], "snpc") * 0.05,  # adjust to control max radius
...
if (file.exists(file.path(fig_dir, "fig4_muscle_motif_enrichment_names.tsv"))) {
  # load file with nice motif names
  mot_nm <- fread(
    file.path(fig_dir, "fig4_muscle_motif_enrichment_names.tsv")
  )
} else {
...
top_dt <- merge.data.table(
  top_dt, mot_nm, by = c("motif", "motif_name"), all.x = TRUE, sort = FALSE, allow.cartesian = TRUE
)
top_dt[, motif_name := NULL]
setnames(top_dt, "motif_name_clean", "motif_name")
...
ggsave(
  file.path(fig_dir, "fig4_muscle_motif_enrichment.pdf"),
  gp_mot_muscl,
  width = 14, height = 5 
)
