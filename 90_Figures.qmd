...
Select example genes to show in the figure.

```{r}
gns <- gene_acc_dt[cell_type == "both"]$gene
pks <- peak_access_dt[category_gene=="both"][category %in% c("other","both")]$peak
cnd_dt <- peak_dt[cell_type %in% cts & peak %in% pks & gene %in% gns]
cnd_dt[,min_gene_score_in_selected_cell_types:=min(gene_score),gene]
cnd_dt <- cnd_dt[min_gene_score_in_selected_cell_types>1.2]
fwrite(cnd_dt, file.path(fig_dir, "example_muscle_genes.csv"))
```

Inspect model contributions for these genes and select the ones we want to show.
...
Which motifs are binding to those peaks?
...
Calculate classic motif enrichment in retractor muscles peaks that are:
...
# load PWMs for experimental motifs and motif archetypes
mta_dir <- "results/Motifs"
arc_dir <- "results/Archetypes"
arc_id <- "PPM-PCC-0.8-IC0.5-5bp"
q <- 0.98
...
# for peaks we show n figure, save moif annots
fav_peaks_genes <- c(
...
blacklist_arch <- c("ARCH1315_M05248_2.00")
motif_2_gene <- unique(mta_dt[,.(gene,gene_name,og,pfam,motif)])
peaks_figure_dt <- rbindlist(lapply(fav_peaks, function(p) {

  print(p)
  
  # first get motifs from scanning
  pm <- as.data.table(
    arc_gr[unique(queryHits(findOverlaps(arc_gr, makeGRangesFromDataFrame(peaks[peak==p]))))]
  )
  setnames(pm, "name", "motif")
  pm[, peak := p]
  pm <- merge.data.table(pm, motif_2_gene, by = "motif")
  pm <- unique(pm[order(start)][,.(peak,motif,start,end,gene,gene_name,og,pfam)])
  pm[, method := "scan"]
  
  # now also check if we have motif from in silico chip
  if (nrow(pm) > 0) {
    pt <- unique(mta_dt[peak==p][in_silico_chip_score>0.1][,.(peak,motif,gene)])
    if (nrow(pt) > 0) {
      pm <- merge.data.table(pm, pt, by = c("peak", "motif", "gene"))
      pm[, method := "insilico_chip"]
    }
  }

  if (!is.null(pm)) {
    pm <- pm[!motif %in% blacklist_arch][, peak_gene := fav_peaks_genes[p]]
    setcolorder(pm, c("peak_gene", "peak"))
  }  
  pm
