...
# gene scores
gene_sco <- readRDS("results/Metacells/cell_type/gene_scores_cell_type.rds")
...
# peaks 
adult_pks <- fread(
  "ArchRProj_Nvec_TSS4_frag200/ArchRProj/Promoters/scATAC_reduced_peaks_cell_type_assignment_filtered.tsv.gz"
)
gastr_pks <- fread(
  "ArchRProj_Nvec_gastrula/ArchRProj/Peaks/scATAC_reduced_peaks_cell_type_assignment.tsv.gz"
)
...
stopifnot(all(num_pks$cell_type %in% colnames(gene_sco)))
num_pks <- num_pks[cell_type %in% colnames(gene_sco)]
...
# gene expression
gene_exp <- readRDS("results/Metacells/cell_type/gene_expression_cell_type_fc.rds")
colnames(gene_exp) <- str_replace_all(colnames(gene_exp), cell_type_rename)
colnames(gene_exp) <- str_remove_all(colnames(gene_exp), "(adult|gastrula)_")
colnames(gene_exp) <- str_replace_all(colnames(gene_exp), "\\.", "_")
```
...
dist_dt[, c("peak", "accessibility") := NULL]
dist_dt <- unique(dist_dt)
dist_dt <- dist_dt[!is.na(expression_fc) & !is.na(gene_score)]
dist_dt <- dist_dt[expression_fc>0 & gene_score>0]

# trim large values
dist_dt[, expression_trim := pmin(expression_fc, 50)]
dist_dt[, gene_score_trim := pmin(gene_score, 50)]

# correlation
cor_exp_sco <- cor(dist_dt$expression_trim, dist_dt$gene_score_trim, method = "spearman")

# scatterplot
cor_gp <- ggplot(dist_dt, aes(expression_trim, gene_score_trim)) +
    geom_point(size = 0.5) +
    geom_smooth(method = "lm", color = "black") +
    labs(
        x = "Gene expression",
        y = "Gene score",
        title = sprintf("Spearman correlation: %.3f", cor_exp_sco)
    ) +
    theme_bw() +
    scale_x_log10(breaks = c(0.1, 1, 10), limits = c(0.05, 50), oob = squish) +
    scale_y_log10(breaks = c(0.1, 1, 10), limits = c(0.05, 20), oob = squish) +
    theme(
...
ggsave(
    file.path(fig_dir, "fig1_gene_expression_score_correlation.pdf"),
    cor_gp,
    width = 4, height = 4
)

# correlation values for all genes
dist_dt[, gene_cor := cor(
    expression_trim, gene_score_trim, method = "spearman"
), by = gene]
dist_cor_dt <- unique(dist_dt[, .(gene, gene_cor)])

# plot distribution 
dist_cor_gp <- ggplot(dist_cor_dt, aes(gene_cor)) +
...
ggsave(
    file.path(fig_dir, "fig1_gene_expression_score_correlation_distribution.pdf"),
    dist_cor_gp,
    width = 4, height = 4
)
...
library(patchwork)
combined_gp <- (cor_gp + dist_cor_gp) +
    plot_layout(nrow = 1, guides = "collect") +
    plot_annotation(
        theme = theme(plot.title = element_text(hjust = 0.5, size = 12))
    )
ggsave(
    file.path(fig_dir, "fig1_gene_expression_score_correlation_combined.pdf"),
    combined_gp,
    width = 6, height = 3
)
```
...
```{bash}
conda create -n scatac -c bioconda -c conda-forge python=3.8
conda activate scatac
conda install -c conda-forge -c bioconda anndata=0.8.0
conda install -c conda-forge -c bioconda scanpy=1.9.1
conda install -c conda-forge -c bioconda matplotlib=3.5.1
```

```{python}
import os
mult_dir="../clustering/SEACells/Results/Multiome/"
atac_fn=os.path.join(mult_dir,"Nvec_Multiome_SEACell_ATAC.h5ad")
rna_fn=os.path.join(mult_dir,"Nvec_Multiome_SEACell_RNA.h5ad")

import anndata as ad
atac=ad.read_h5ad(atac_fn)
rna=ad.read_h5ad(rna_fn)
...
if (is.null(ap_dir)) {
  class_peaks_adult <- fread(file.path(adult_pks_dir, "annotation_promoters_full.tsv.gz"))
  ...
  class_peaks_gastr <- fread(file.path(gastr_pks_dir, "annotation_promoters_full.tsv.gz"))
  ...
  class_peaks <- fread(file.path(ap_dir, "annotation_promoters_full.tsv.gz"))[, stage := stage]
}
...
gl_gp_2 <- ggplot(gl_dt_2, aes(genelist_full, num_non_prom)) +
  ...
  #stat_compare_means(comparisons = list(c("chromatin", "metabolism")), label = "p.signif", method = "wilcox.test") +
  stat_compare_means(
...
phastc_list <- lapply(
  names(phylop_fn), function(x) {
    ...
    if (x == "CRE") {
      ...
      ovl <- findOverlaps(query = cons_gr, subject = maps_gr, minoverlap = 150)
...
# types of CREs
class_adult <- fread(file.path(adult_pks_dir, "annotation_promoters_full.tsv.gz"))
class_gastr <- fread(file.path(gastr_pks_dir, "annotation_promoters_full.tsv.gz"))
...
gp_ph <- ggplot(
  ...
  scale_color_manual(
    values = structure(colorspace::darken(ph_cols, 0.5), names = names(ph_cols)), 
    # hide in legend
    guide = "none"
  ) +
  scale_y_continuous(
...
Plot phyloP per cell type

```{r}
class_peaks_gr <- makeGRangesFromDataFrame(class_peaks, keep.extra.columns = TRUE)
phastc_cre_gr <- makeGRangesFromDataFrame(phastc_dt, keep.extra.columns = TRUE)
ovl <- findOverlaps(query = class_peaks_gr, subject = phastc_cre_gr, ignore.strand = TRUE, minoverlap = 200)

ph_ct <- phastc_cre_gr[subjectHits(ovl)]
pk_ct <- class_peaks_gr[queryHits(ovl)]
pk_ct$phylop_score <- ph_ct$phylop_score
pk_ct <- unique(as.data.table(pk_ct))

pk_gp <- unique(pk_ct[, .(seqnames, start, end, stage, cell_type, peak, Log2FC, Log2FC_like, phylop_score)])
setorder(pk_gp, seqnames, start, end, -phylop_score)
pk_gp <- pk_gp[,  .SD[1], .(peak, stage, cell_type)]

pk_gp[cell_type == "gastro_unknown", cell_type := "gastro"]

all_cts <- unique(unname(unlist(sapply(
  names(bct_cols), function(x) c(x, grep(x, names(ct_cols), value=TRUE))
))))
all_cts_cols <- c(bct_cols, ct_cols)[all_cts]
stopifnot(all(pk_gp$cell_type %in% all_cts))
pk_gp <- pk_gp[!is.na(cell_type)]
pk_gp[, cell_type := factor(cell_type, levels = all_cts)]

# binarize phyloP scores
pk_gp[, pval := 10^ (-1 * abs(phylop_score))]
pk_gp[, evol := "neutral"]
pk_gp[phylop_score > 0 & pval < 0.001, evol := "slow-evolving"]
pk_gp[phylop_score < 0 & pval < 0.001, evol := "acceleration"]
pk_gp[, evol := factor(evol, levels = c("neutral", "acceleration", "slow-evolving"))]
setorder(pk_gp, evol, cell_type)
pk_gp[, evol := paste(cell_type, evol)]
pk_gp[, evol2 := paste0(evol, "2")]
pk_gp[, evol := factor(evol, levels = unique(pk_gp$evol))]

# remove peaks with 0 phylop score
# pk_gp <- pk_gp[phylop_score != 0]

# filter cell type specific peaks
filt <- FALSE
if (filt) {
  pk_gp <- pk_gp[(Log2FC>2 | Log2FC_like>2)]
  suff <- "_filt"
} else {
  suff <- ""
}

# count peaks per cell type and evolution
pk_bn <- pk_gp[, .N, .(cell_type, evol, evol2)]
pk_bn[, perc := N / sum(N) * 100, cell_type]
pk_bn[, label := sprintf("%.1f%%", perc)]

evol_cols <- structure(c(
  colorspace::darken(all_cts_cols, 0.4),
  unname(all_cts_cols),
  colorspace::lighten(all_cts_cols, 0.5)
), names = c(
  paste(names(all_cts_cols), "slow-evolving"),
  paste(names(all_cts_cols), "neutral"),
  paste(names(all_cts_cols), "acceleration")
))

# plot fraction of peaks with accel./slow evol as pie chart
gp_pie <- ggplot(
  pk_bn,
  aes(x = "", y = perc, fill = evol)
) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  scale_fill_manual(values = evol_cols) +
  coord_polar(theta = "y") +
...
ggsave(
  file.path(fig_dir, sprintf("phylop_scores_per_cell_type%s_pie.pdf", suff)),
  gp_pie,
  width = 10, height = 6
)

# plot fraction of peaks with accel./slow evol as stacked barplot
pk_bn[, evol := factor(evol, levels = c(
  paste(all_cts, "acceleration"),
  paste(all_cts, "neutral"),
  paste(all_cts, "slow-evolving")
))]
gp_pie <- ggplot(
  pk_bn,
  aes(x = perc, y = cell_type, fill = evol)
) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  scale_fill_manual(values = evol_cols) +
  theme(legend.position = "none") +
...
ggsave(
  file.path(fig_dir, sprintf("phylop_scores_per_cell_type%s_stack.pdf", suff)),
  gp_pie,
  width = 10, height = 6
)

# plot fraction of peaks with accel./slow evol as barplot
gp_bar <- ggplot(
    pk_bn[!grepl("neutral", evol)],
    aes(cell_type, perc, fill = evol, color = cell_type)
  ) +
  geom_bar(stat = "identity", position = position_dodge(preserve = "single")) +
  scale_fill_manual(values = evol_cols) +
  scale_color_manual(
    values = structure(colorspace::darken(all_cts_cols, 0.5), names = names(all_cts_cols)), 
    guide = "none"
  ) +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.05))
  ...
  labs(x = "", y = "% of peaks")
ggsave(
  file.path(fig_dir, sprintf("phylop_scores_per_cell_type%s_evol.pdf", suff)),
  gp_bar,
  width = 7, height = 4
)

# plot distribution of phylop scores
pk_box <- ggplot(
   pk_gp,
    aes(cell_type, phylop_score, fill = cell_type, color = cell_type)
  ) +
  geom_violin(width = 1.2, alpha = 0.2) +
  ggbeeswarm::geom_quasirandom(size = 1, dodge.width = 0.9) +
  #geom_boxplot(outlier.shape = NA) +
  scale_fill_manual(values = all_cts_cols) +
  scale_color_manual(values = all_cts_cols) +
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
    panel.grid.major.x = element_line(linewidth = 0.5),
...
ggsave(
  file.path(fig_dir, sprintf("phylop_scores_per_cell_type%s_dist.pdf", suff)),
  pk_box,
  width = 8, height = 5
)
...
For muscle sets of peaks

```{r}
pks_phc <- fread(file.path(con_dir, "phastcons.Nvec.phylop_per_peak.bed"), fill = TRUE)
setnames(pks_phc, c("seqnames", "start", "end", "name", "score", "strand"))

peaks_dir <- file.path("results", "Motifs", "streme", "muscle")
pks_both <- fread(file.path(peaks_dir, "peaks_both.bed"))
setnames(pks_both, c("seqnames", "start", "end", "name", "score", "strand"))
pks_tr <- fread(file.path(peaks_dir, "peaks_muscle_TR.bed"))
setnames(pks_tr, c("seqnames", "start", "end", "name", "score", "strand"))
pks_mr <- fread(file.path(peaks_dir, "peaks_muscle_MR.bed"))
setnames(pks_mr, c("seqnames", "start", "end", "name", "score", "strand"))

pks_phc_gr <- makeGRangesFromDataFrame(pks_phc, keep.extra.columns = TRUE)
pks_both_gr <- makeGRangesFromDataFrame(pks_both, keep.extra.columns = TRUE)
pks_tr_gr <- makeGRangesFromDataFrame(pks_tr, keep.extra.columns = TRUE)
pks_mr_gr <- makeGRangesFromDataFrame(pks_mr, keep.extra.columns = TRUE)

ovl_both <- findOverlaps(pks_phc_gr, pks_both_gr, minoverlap = 50, ignore.strand = TRUE)
ovl_tr <- findOverlaps(pks_phc_gr, pks_tr_gr, minoverlap = 50, ignore.strand = TRUE)
ovl_mr <- findOverlaps(pks_phc_gr, pks_mr_gr, minoverlap = 50, ignore.strand = TRUE)

pks_both_phc <- pks_both[subjectHits(ovl_both)]
pks_tr_phc <- pks_tr[subjectHits(ovl_tr)]
pks_mr_phc <- pks_mr[subjectHits(ovl_mr)]

pks_both_phc$phastCon_score <- pks_phc_gr[queryHits(ovl_both)]$score
pks_tr_phc$phastCon_score <- pks_phc_gr[queryHits(ovl_tr)]$score
pks_mr_phc$phastCon_score <- pks_phc_gr[queryHits(ovl_mr)]$score

setorder(pks_both_phc, -phastCon_score)
setorder(pks_tr_phc, -phastCon_score)
setorder(pks_mr_phc, -phastCon_score)

pks_both_phc <- pks_both_phc[, .SD[1], .(seqnames, start, end)]
pks_tr_phc <- pks_tr_phc[, .SD[1], .(seqnames, start, end)]
pks_mr_phc <- pks_mr_phc[, .SD[1], .(seqnames, start, end)]

pks_dt_rt <- rbindlist(list(
  "both" = pks_both_phc,
  "muscle_tentacle_retractor" = pks_tr_phc,
  "muscle_mesentery_retractor" = pks_mr_phc
), idcol = "set")

pks_dt_rt[, phastCon_sign := sign(phastCon_score)]
pks_dt_rt[, phastCon_sign := factor(phastCon_sign, levels = c("-1", "0", "1"))]

pks_gg_rt <- ggplot(pks_dt_rt, aes(set, fill = set)) +
  geom_bar(position = "dodge") +
  facet_grid(~ phastCon_sign, scales = "free_y") +
  scale_fill_manual(values = c(ct_cols, "both" = "#f9e655")) +
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
    legend.position = "none"
  )
...
ggsave(
  file.path(fig_dir, "fig4_phastcon.pdf"),
  pks_gg_rt,
  height = 6, width = 3
)

setnames(pks_dt_rt, "phastCon_score", "phylop_score")
pks_dt_rt[, pval := 10^ (-1 * abs(phylop_score))]
pks_dt_rt[, evol := "neutral"]
pks_dt_rt[phylop_score < 0 & pval < 0.001, evol := "slow-evolving"]
pks_dt_rt[phylop_score > 0 & pval < 0.001, evol := "acceleration"]
pks_dt_rt[, evol := factor(evol, levels = c("neutral", "acceleration", "slow-evolving"))]
setorder(pks_dt_rt, evol, type)
...
Plot phastCon scores per cell type

```{r}
ovl <- findOverlaps(query = class_peaks_gr, subject = phast_gr, ignore.strand = TRUE)
ovl_gr <- phast_gr[subjectHits(ovl)]
cls_gr <- class_peaks_gr[queryHits(ovl)]
mcols(cls_gr)$score <- mcols(ovl_gr)$score
ps_dt <- unique(as.data.table(cls_gr)[, c("gene", "umifrac", "mcfp") := NULL])

# data to plot
bin_vals <- function(x, w = 100, agg_fun = max) {
  if (length(x) < w) {
    return(agg_fun(x))
  } else {
    starts <- seq(1, w, by = 1)
    ...
    for (i in seq_along(starts)) {
      ...
      vals <- x[starts[i]:ends[i]]
      out[i] <- agg_fun(vals)
    }
    return(out)
  }
}
#cons_plot_dt <- ps_dt[, .(score = max(bin_vals(score, w=100, agg_fun=sum))), .(cell_type, peak, Log2FC, Log2FC_like, stage)]
cons_plot_dt <- ps_dt[, .(score = max(score)), .(cell_type, peak, Log2FC, Log2FC_like, stage)]

# order cell types
cons_plot_dt[cell_type == "gastro_unknown", cell_type := "gastro"]
stopifnot(all(cons_plot_dt$cell_type %in% all_cts))
cons_plot_dt[, cell_type := factor(cell_type, levels = all_cts)]
setorder(cons_plot_dt, cell_type, -score)

# filter cell type specific peaks
cons_plot_dt <-  cons_plot_dt[(Log2FC>2 | Log2FC_like>2)]

# plot phastCon scores
gp_phastc_ovl <- ggplot(
    cons_plot_dt,
    aes(cell_type, score, color = cell_type, fill = cell_type)
  ) +
  geom_violin(width = 1.2, alpha = 0.2) +
  #ggbeeswarm::geom_quasirandom(size = 0.2, dodge.width = 0.9) +
  #geom_boxplot(outlier.shape = NA) +
  scale_y_continuous(
    #trans = "log10"
    #limits = c(0, 1), oob = scales::squish,
  ...
  scale_fill_manual(values = all_cts_cols) +
  scale_color_manual(values = structure(
    colorspace::darken(all_cts_cols, 0.5),
    names = names(all_cts_cols)
  )) +
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
# save
ggsave(
  file.path(fig_dir, "phylop_phastcon_scores_cell_types_w100.pdf"),
  gp_phastc_ovl,
  width = 4, height = 4
)
```
...
## Peaks dynamics (this ended being moved to Figure 1)
...
ggsave(file.path(fig_dir, "fig1_gene_scores_distribution.pdf"), width = 8, height = 5)
...
gns_ovl_dt <- gns_ovl_dt[!is.na(adult) & !is.na(gastrula)]
gns_ovl_dt[gene %in% gene_acc_dt[stage == "both"]$gene, c("adult", "gastrula") := TRUE]
gns_ovl_mt <- as.matrix(gns_ovl_dt[, -1])

require(eulerr)
stage_cols <- c("adult" = "#fd9b44", "gastrula" = "#fdf6ab", "both" = "#fed38d")
...
pdf(
  file.path(fig_dir, "fig1_genes_adult_gastrula_ovl.pdf"),
  width = 4, height = 2
)
print(p1)
dev.off()
...
ggsave(file.path(fig_dir, "fig1_gene_expression_distribution.pdf"), gene_exp_dist, width = 8, height = 5)
...
gene_exp_scat <- ggplot(exp_dt, aes(gastr_exp, adult_exp, color = category)) +
  ...
  coord_fixed()
ggsave(
  file.path(fig_dir, sprintf("fig1_gene_expression_scatterplot_%s.pdf", thrs_umi)),
...
pdf(
  file.path(fig_dir, sprintf("fig1_genes_adult_gastrula_exp_ovl_%s.pdf", thrs_umi)),
  width = 4, height = 2
)
print(p1)
dev.off()
...
adult_pks <- fread(file.path(adult_dir, "ArchRProj", "Promoters", "scATAC_reduced_peaks_gene_assignment_coaccess.tsv.gz"))
adult_pks <- unique(adult_pks[, .(peak, gene)])
setnames(adult_pks, "peak", "peak_adult")
gastr_pks <- fread(file.path(gastr_dir, "ArchRProj", "Peaks", "scATAC_reduced_peaks_gene_assignment_coaccess.tsv.gz"))
...
pks_map <- pks_map_all[, .(peak, peak_adult, peak_gastrula)]
...
ggsave(
  file.path(fig_dir, sprintf("fig1_dynamics_peak_%s.pdf", thrs_umi)),
  gp_dynm, width = 6, height = 4
)
```

Homer motif enrichment in dynamic peaks

```{r}
set.seed(1950)
for (gs in c("gastrula", "adult", "both")) {
  for (ps in c("gastrula", "adult", "both")) {
      pks_dt <- dynm_acc_dt[gene_stage == gs & peak_stage == ps & promoter == "NO"]
      pks <- unique(pks_dt$peak)
      print(sprintf("%s peaks open in %s and assocaited to gene open in %s", length(pks), ps, gs))
      if (length(pks) > 0) {
        pks_bed <- pks_map_all[peak %in% pks][, .(seqnames, start, end, peak, score, strand)]
        fwrite(
          pks_bed,
          file.path(res_dir, "Motifs", "homer", "adult_gastrula", sprintf("peaks_%s_genes_%s.bed", ps, gs)),
          sep = "\t",
          col.names = FALSE
        )
        fwrite(
          pks_map_all[!peak %in% pks][, .(seqnames, start, end, peak, score, strand)][sample(1:.N, nrow(pks_bed))],
          file.path(res_dir, "Motifs", "homer", "adult_gastrula", sprintf("peaks_%s_genes_%s_bg.bed", ps, gs)),
          sep = "\t",
          col.names = FALSE
        )
...
mta_dap <- readRDS(
  file.path(mta_dir, "dapseq-motifs.rds")
)
arc_pwm <- readRDS(
  file.path(arc_dir, sprintf("motif-archetypes-%s-pwms.rds", arc_id))
)
pwms <- c(mta_pwm, mta_dap, arc_pwm[!names(arc_pwm) %in% names(mta_pwm)])
...
gs <- c("adult", "both")
    ...
    ct <- sprintf("peaks_%s_genes_%s", ps, paste(gs, collapse="_"))
    pks_dt <- dynm_acc_dt[gene_stage %in% gs & peak_stage %in% ps & promoter == "NO"]
    pks <- unique(pks_dt$peak)
    print(sprintf("%s peaks open in %s and assocaited to gene open in %s", length(pks), ps, gs))
      
    if (length(pks) > 0) {
      # select and save peaks
      pks_bed <- pks_map_all[peak %in% pks][, .(seqnames, start, end, peak, score, strand)]
      #fwrite(
      #  pks_bed,
      #  file.path(res_dir, "Motifs", "homer", "adult_gastrula", sprintf("peaks_%s_genes_%s.bed", ps, gs)),
      ...
      #  col.names = FALSE
      #)
      # for background, subset from all peaks that are not used in dynamic peaks quantification
      bgs <- peaks[!peak %in% dynm_acc_dt$peak]$peak
      pks_bgd <- pks_map_all[peak %in% bgs][, .(seqnames, start, end, peak, score, strand)][sample(1:.N, nrow(pks_bed))]
      # for background, subset from all peaks that are not in the foreground
      pks_bgd <- pks_map_all[!peak %in% pks][, .(seqnames, start, end, peak, score, strand)][sample(1:.N, nrow(pks_bed))]
      #fwrite(
      #  pks_bgd,
      #  file.path(res_dir, "Motifs", "homer", "adult_gastrula", sprintf("peaks_%s_genes_%s_bg.bed", ps, gs)),
      #  sep = "\t",
      ...
      # foreground peaks
      pks_fg <- makeGRangesFromDataFrame(pks_bed)

      # background peaks
      pks_bg <- makeGRangesFromDataFrame(pks_bgd)
...
enr_dt <- enr_dt[motif %in% names(pwms)]

# remove degenerate motif
enr_dt <- enr_dt[!grepl("ARCH895_", motif)]

# save results archetypes
fwrite(
  enr_dt,
  file.path(res_dir, "Motifs", sprintf("motif-enrichment-dynamic-peaks-adult-gastrula.tsv.gz")),
  sep = "\t"
)

# significant results
enr_top <- enr_dt[fc>1 & padj<0.05]

enr_fc <- dcast.data.table(enr_dt[motif %in% enr_top$motif][, .(motif, fc, padj, label)], motif~label, value.var="fc")
enr_padj <- dcast.data.table(enr_dt[motif %in% enr_top$motif][, .(motif, fc, padj, label)], motif~label, value.var="padj")
...
saveRDS(
  sim_mat, 
  file.path(res_dir, "Motifs", sprintf(
    "dynamic-peaks-adult-gastrula-motifs-similarity-%s-%s.rds", similarity, method
  ))
)
...
# enrichment heatmap
mat_fc <- as.matrix(enr_fc[rownames(sim_mat), -1])
rownames(mat_fc) <- rownames(sim_mat)
mat_fc <- log2(mat_fc)
min_fc <- min(mat_fc[!is.infinite(mat_fc)])
mat_fc[is.infinite(mat_fc)] <- min_fc
mat_padj <- as.matrix(enr_padj[rownames(sim_mat), -1])
rownames(mat_padj) <- rownames(sim_mat)
mat_padj <- -1*log10(mat_padj)

col_fun <- colorRamp2(
  c(pmin(-2, min(mat_fc, na.rm = TRUE)), 0, max(2, max(mat_fc, na.rm = TRUE))),
  c("#a6611a", "#f5f5f5", "#018571")
)
hm_enr <- Heatmap(
  mat_fc, name = "enrichment\nlog2FC",
  col = col_fun,
  rect_gp = gpar(type = "none"),
  width = unit(1, "null"),
  cell_fun = function(j, i, x, y, width, height, fill) {
    # Draw dots
    scaled_size = pmin(mat_padj / 3, 3)  # cap
    grid.circle(
      x = x, y = y,
      r = unit(scaled_size[i, j], "snpc") * 0.1,  # adjust to control max radius
      gp = gpar(fill = col_fun(mat_fc[i, j]), col = NA)
    )
    # Add significance stars
    pval <- 10^(-mat_padj[i, j])  # reverse -log10
    signif_symbol <- ""
    if (!is.na(pval)) {
      if (pval < 0.01) {
        signif_symbol <- "*"
    ...
    if (signif_symbol != "") {
      grid.text(signif_symbol, x = x, y = y, gp = gpar(fontsize = 6))
    }
  },
  border = TRUE,
  cluster_rows = FALSE, 
...
# save heatmap
pdf(
  file.path(fig_dir, "fig1_dynamic_peaks_adult_gastrula_motif_similarity_heatmap.pdf"),
  width = 17, height = 12
)
draw(hm_sim + hm_enr)
dev.off()
...
# save table
mat_dt <- data.table(motif = rownames(mat_fc))
for (cn in colnames(mat_fc)) {
  colname <- paste0("fc_", cn)
  mat_dt[, I(colname) := mat_fc[,cn]]
}
for (cn in colnames(mat_padj)) {
  colname <- paste0("minuslog10_padj_", cn)
  mat_dt[, I(colname) := mat_fc[,cn]]
}
fwrite(
  mat_dt,
  file.path(fig_dir, "fig1_dynamic_peaks_adult_gastrula_motif_similarity.tsv"),
  sep = "\t"
)

# save motif logos
pdf(
  file.path(fig_dir, "fig1_dynamic_peaks_adult_gastrula_motif_enrichment_logos.pdf"),
...
selected_motifs <- c(
"Usf3"       = "ARCH1570_Usf3",
"Sox4/11/13" = "ARCH1164_/Sox4/Sox11_Sox13/...",
"bHLH"       = "ARCH471_BHLHE22__NHLH1__Ptf...",
"Tbx/Eomes"  = "ARCH1507_Eomes",
"E2f"        = "ARCH1222_/E2f4_E2f6_E2f8/E2f7",
"Alx"        = "ARCH227_Alx3/Alx4/Alx1_Emx1...",
"Arid"       = "ARCH1447_Arid5b/Arid5a",
"Pou3f3"     = "ARCH1214_POU3F3",
"Nr2e3"      = "ARCH1411_Nr2e3",
"Gata1/2/3"  = "ARCH1478_Gata3/Gata1/Gata2"
)

selected_motifs_sorted <- names(sort(mat_padj[selected_motifs,],decreasing=TRUE))
names(selected_motifs_sorted) <- names(selected_motifs)[match(selected_motifs_sorted, selected_motifs)]

mat_fc_selected <- mat_fc[selected_motifs_sorted,,drop=FALSE]
mat_padj_selected <- mat_padj[selected_motifs_sorted,,drop=FALSE]
sim_mat_selected <- sim_mat[selected_motifs_sorted,selected_motifs_sorted,drop=FALSE]

rownames(mat_fc_selected) <- names(selected_motifs_sorted)
rownames(mat_padj_selected) <- names(selected_motifs_sorted)
rownames(sim_mat_selected) <- names(selected_motifs_sorted)

# similarity heatmap
col_vals <- c(0,0.5,1)  
col <- colorRamp2(col_vals, c("#67a9cf", "#f7f7f7", "#ef8a62"))
hm_sim <- ComplexHeatmap::Heatmap(
  sim_mat_selected, name = "motif\nsimilarity",
  width = unit(12, "null"),
  col = col,
  cluster_rows = FALSE, 
  cluster_columns = FALSE,
...
col_fun_selected <- colorRamp2(
  c(-1.5, 0, 1.5),
  c("#a6611a", "#f5f5f5", "#018571")
)
mat_fc_selected <- pmin(mat_fc_selected, 1.5)
mat_fc_selected <- pmax(mat_fc_selected, -1.5)
hm_enr <- Heatmap(
  mat_fc_selected, name = "enrichment\nlog2FC",
  col = col_fun_selected,
  rect_gp = gpar(type = "none"),
  width = unit(2, "null"),
  cell_fun = function(j, i, x, y, width, height, fill) {
    # Draw dots
    scaled_size = pmin(mat_padj_selected / 3, 3)  # cap
    grid.circle(
      x = x, y = y,
      r = unit(scaled_size[i, j], "snpc") * 0.2,  # adjust to control max radius
      gp = gpar(fill = col_fun_selected(mat_fc_selected[i, j]), col = NA)
    )
    # Add significance stars
    pval <- 10^(-mat_padj_selected[i, j])  # reverse -log10
    signif_symbol <- ""
    if (!is.na(pval)) {
      if (pval < 0.01) {
        signif_symbol <- "*"
  ...
  column_names_gp = gpar(fontsize = 8),
  row_names_max_width = max_text_width(
      rownames(sim_mat_selected), 
      gp = gpar(fontsize = 8)
  ),
  column_names_max_height = max_text_width(
      colnames(sim_mat_selected), 
      gp = gpar(fontsize = 8)
  )
)
...
# save heatmap
pdf(
  file.path(fig_dir, "fig1_dynamic_peaks_adult_gastrula_motif_similarity_heatmap_selected.pdf"),
  width = 5, height = 4
)
draw(hm_sim + hm_enr)
dev.off()
...
# save motif logos
stopifnot(all(selected_motifs_sorted %in% names(pwms)))
enr_pwms_selected <- lapply(seq_along(selected_motifs_sorted), function(i) {
  x <- selected_motifs_sorted[i]
  n <- names(selected_motifs_sorted)[i]
  pwm <- pwms[[x]]
  pwm@name <- n
  pwm
})
...
pdf(
  file.path(fig_dir, "fig1_dynamic_peaks_adult_gastrula_motif_enrichment_logos_selected.pdf"),
  width = 4, height = length(enr_pwms_selected) * 0.8
)
view_motifs(enr_pwms_selected)
dev.off()
...
```{r}
grn_dir <- file.path(res_dir, "GRN", "networks")
lvl <- "metacell"
thrs <- 0.1
exp <- "umifrac"
acc <- "access"
...
```{r}
# load nodes annotation
data_dir <- file.path(res_dir, "GRN/networks/")
nodes_ann <- fread(file.path(
  data_dir, "broad_cell_type", 
  "cartography_broad_cell_type_expression_fc_1.2_accessibility_access_0.4_chromvar_4.tsv"
))

# where to save plots
pdf(file.path(
  fig_dir, "grn_metrics_broad_cell_type_expression_fc_1.2_accessibility_access_0.4_chromvar_4.pdf"
), height = 8, width = 32)
...
```{r}
nodes_ann <- fread(file.path(
  grn_dir, "broad_cell_type",
  "cartography_broad_cell_type_expression_fc_0.4_accessibility_access_0.4_chromvar_4.tsv"
))

# how many modules per GRN?
nodes_ann[,.N,.(cell_type,module)][,.N,cell_type]

# overlap modules
setorder(nodes_ann, cell_type)
nodes_ann[, module := paste0(cell_type, "_M", module)]
...
Overlap of TFs between neuron and cnidocytes

```{r}
grns <- fread("results/GRN/networks/broad_cell_type/grn_genes_broad_cell_type_expression_fc_0.4_accessibility_access_0.4_chromvar_4.tsv.gz")

cnido_grn <- grns[cell_type=="cnidocyte"]
neuro_grn <- grns[cell_type=="neuron_Pou4_FoxL2"]

# overlap of TFs between cnidocytes and neuron Pou4
cnido_neuro_tfs <- intersect(cnido_grn$gene, neuro_grn$gene) # 34
cnido_only_tfs <- setdiff(cnido_grn$gene, neuro_grn$gene) # 20
neuro_only_tfs <- setdiff(neuro_grn$gene, cnido_grn$gene) # 57

# plot euler diagram
library(eulerr)
euler_vals <- c(
  "cnidocyte" = length(cnido_only_tfs),
  "neuron_Pou4_FoxL2" = length(neuro_only_tfs),
  "cnidocyte&neuron_Pou4_FoxL2" = length(cnido_neuro_tfs)
)
fit <- euler(euler_vals)
p1 <- plot(
  fit,
  quantities = TRUE,
  labels = list(font = 4),
  fill = c(bct_cols, "cnidocyte&neuron_Pou4_FoxL2" = "purple")[names(euler_vals)]
)
pdf(file.path(
  fig_dir, "GRN_cnido_neuro_TFs_euler.pdf"
), height = 4, width = 6)
print(p1)
dev.off()

cnido_neuro_tfs_dt <- data.table(
  gene = c(cnido_neuro_tfs, cnido_only_tfs, neuro_only_tfs),
  group = rep(
    c("cnidocyte&neuron", "cnidocyte_only", "neuron_only"), 
    c(length(cnido_neuro_tfs), length(cnido_only_tfs), length(neuro_only_tfs)
  ))
)
cnido_neuro_tfs_dt <- merge.data.table(
  cnido_neuro_tfs_dt,
  tfan[, .(gene, common_name, gene_name, og)],
  by = "gene", all.x = TRUE, sort = FALSE
)
fwrite(
  cnido_neuro_tfs_dt, 
  file.path(fig_dir, "GRN_cnido_neuro_Pou4_FoxL2_tfs.tsv"), 
  sep = "\t"
)
```
...
# enrichment for each group of TF targets
for (grp in c("cnidocyte&neuron", "neuron_only", "cnidocyte_only")) {

  # where to save results
  gos_dir <- file.path("results", "GRN", "GO")
  dir.create(gos_dir, recursive = TRUE, showWarnings = FALSE)

  tfs <- cnido_neuro_tfs_dt[group == grp]$gene
  cdts <- grns[cell_type %in% c("cnidocyte", "neuron_Pou4_FoxL2")]
  gdts <- cdts[gene %in% tfs]

  genes_fg <- unique(gdts$target_gene)
  genes_bg <- unique(cdts$target_gene)
  message(sprintf(
      "%s target genes; %s background genes for %s",
      length(genes_fg), length(genes_bg), grp
  ))
  ...
  gos_en <- gsa_topgo_enrichment(
      ...
      genes_fg = genes_fg,
      genes_bg = genes_bg,
      output_prefix = file.path(gos_dir, sprintf("GO.%s.", grp)),
      name_fg = grp,
...
gos_dt <- rbindlist(gos_ls, idcol = "group")
fwrite(
  gos_dt,
  file.path("results", "GRN", "GO", "GO_enrichment.tsv.gz"), 
  sep = "\t", compress = "gzip"
)
pvalthr <- 0.05
fwrite(
  gos_dt[pval_test<pvalthr],
  file.path("results", "GRN", "GO", sprintf("GO_enrichment_pval_%s.tsv.gz", pvalthr)), 
  sep = "\t", compress = "gzip"
)

```

Where are those TFs in the hierarchy of TFs?

For cnidocyte:

```{r} 
# Subset the GRN to cnidocyte TFs
cnido_tfs_grn <- cnido_grn[target_gene %in% cnido_grn$gene]
cnido_tfs_grn[, tf_type := ifelse(
  gene %in% cnido_neuro_tfs, "cnidocyte & neuron", "cnidocyte_only"
), by = .I]

# Make labels
name_dt <- unique(rbind(
  cnido_tfs_grn[, .(id = gene, common = fifelse(common_name != "", common_name,
                                      fifelse(gene_name != "", gene_name,
                                      fifelse(og != "", og, gene))))],
  cnido_tfs_grn[, .(id = target_gene, common = fifelse(target_common_name != "", target_common_name,
                                            fifelse(target_gene_name != "", target_gene_name,
                                            fifelse(og != "", og, target_gene))))]  
))
name_dt[, common := substr(common, 1, 12)]

gene_cnido_list <- list()

for (tf_g in unique(cnido_tfs_grn$gene)) {

  tf_g_dt <- cnido_tfs_grn[
    gene == tf_g | target_gene == tf_g, 
    .(gene, target_gene, tf_type, expression_fc, target_expression_fc, zscore, in_silico_chip_score)
  ]
  ...
  # Assign edge attributes (line width)
  E(g)$in_silico_chip_score <- tf_g_dt$in_silico_chip_score

  # Combine gene and target_gene expression_fc values into a node-wise summary
  node_fc_dt <- unique(rbind(
    tf_g_dt[, .(name = gene, expression_fc, zscore)],
    tf_g_dt[, .(name = target_gene, expression_fc = target_expression_fc, zscore = 1)]
  ))

  # Match values to nodes
  V(g)$expression_fc <- node_fc_dt[match(V(g)$name, node_fc_dt$name), expression_fc]
  V(g)$zscore <- node_fc_dt[match(V(g)$name, node_fc_dt$name), zscore]

  # Cap very high values
  V(g)$expression_fc <- pmin(pmax(V(g)$expression_fc, 0), 6)
  V(g)$zscore <- pmin(pmax(V(g)$zscore, 0.5), 30)

  # Color by type of TF
  V(g)$tf_type <- tf_g_dt[match(V(g)$name, tf_g_dt$gene), tf_type]
  V(g)$tf_type[is.na(V(g)$tf_type)] <- "-"

  # Map labels
  V(g)$label <- name_dt[match(V(g)$name, name_dt$id), common]

  gene_cnido_list[[tf_g]] <- ggraph(g, layout = 'tree') +
    geom_edge_link(aes(width = in_silico_chip_score), color = "grey" ,
                  arrow = arrow(length = unit(3, 'mm'), type = "open"),
                  end_cap = circle(5, 'mm'), alpha = 0.8) +
    geom_node_point(aes(size = zscore, fill = expression_fc, color = tf_type),
                    shape = 21, stroke = 1.2) +
    geom_node_text(aes(label = label), repel = TRUE, size = 3, vjust = 1.5) +
    scale_fill_gradientn(
      name = "expression\nfold change",
      breaks = c(0, 2, 4, 6, 8),
      colours = c(
    ...
    scale_color_manual(values = c(
      "cnidocyte_only" = unname(bct_cols["cnidocyte"]),
      "neuron_only" = unname(bct_cols["neuron_Pou4_FoxL2"]),
      "cnidocyte & neuron" = "purple",
      "-" = "white"
    )) +
    scale_edge_width(range = c(0.2, 2)) +
    scale_size_continuous(range = c(2, 10)) +
    theme_void() +
    ggtitle(sprintf("Cnidocyte TF network for %s (%s)", name_dt[id==tf_g]$common, tf_g))

}

pdf(file.path(fig_dir, "GRN_cnido_TFs_hierarchy.pdf"), height = 12, width = 18)
print(gene_cnido_list)
dev.off()
...
```{r} 
# Subset the GRN to neuron TFs
neuro_tfs_grn <- neuro_grn[target_gene %in% neuro_grn$gene]
neuro_tfs_grn[, tf_type := ifelse(
  gene %in% cnido_neuro_tfs, "cnidocyte & neuron", "neuron_only"
), by = .I]

# Make labels
name_dt <- unique(rbind(
  neuro_tfs_grn[, .(id = gene, common = fifelse(common_name != "", common_name,
                                      fifelse(gene_name != "", gene_name,
                                      fifelse(og != "", og, gene))))],
  neuro_tfs_grn[, .(id = target_gene, common = fifelse(target_common_name != "", target_common_name,
                                            fifelse(target_gene_name != "", target_gene_name,
                                            fifelse(og != "", og, target_gene))))]  
))
name_dt[, common := substr(common, 1, 12)]

gene_neuro_list <- list()

for (tf_g in unique(neuro_tfs_grn$gene)) {

  tf_g_dt <- neuro_tfs_grn[
    gene == tf_g | target_gene == tf_g, 
    .(gene, target_gene, tf_type, expression_fc, target_expression_fc, zscore, in_silico_chip_score)
  ]
  ...
  gene_neuro_list[[tf_g]] <- ggraph(g, layout = 'tree') +
    geom_edge_link(aes(width = in_silico_chip_score), color = "grey" ,
                  arrow = arrow(length = unit(3, 'mm'), type = "open"),
                  end_cap = circle(5, 'mm'), alpha = 0.8) +
    geom_node_point(aes(size = zscore, fill = expression_fc, color = tf_type),
    ...
    scale_size_continuous(range = c(2, 10)) +
    theme_void() +
    ggtitle(sprintf("Neuron TF network for %s (%s)", name_dt[id==tf_g]$common, tf_g))

}

pdf(file.path(fig_dir, "GRN_neuro_TFs_hierarchy.pdf"), height = 12, width = 18)
print(gene_neuro_list)
dev.off()

```

Compare overlap of TF regulomes for shared TFs

```{r}
jaccs_all <- sapply(cnido_neuro_tfs, function(tf_g) {
  cnido_targets <- cnido_grn[gene == tf_g, target_gene]
  neuro_targets <- neuro_grn[gene == tf_g, target_gene]
  length(intersect(cnido_targets, neuro_targets)) / length(union(cnido_targets, neuro_targets))
})

jaccs_tfs <- sapply(cnido_neuro_tfs, function(tf_g) {
  cnido_targets <- cnido_grn[gene == tf_g & target_TF == TRUE, target_gene]
  neuro_targets <- neuro_grn[gene == tf_g & target_TF == TRUE, target_gene]
  length(intersect(cnido_targets, neuro_targets)) / length(union(cnido_targets, neuro_targets))
})

# boxplots
jacc_dt <- data.table(
  jacc = c(jaccs_all, jaccs_tfs),
  type = rep(c("all", "TFs"), each = length(jaccs_all))
)
gp_jacc <- ggplot(jacc_dt, aes(type, jacc)) +
  geom_boxplot() +
  geom_jitter(width = 0.2) +
  labs(x = "Type of targets", y = "Jaccard index") +
  theme_minimal()

# save 
ggsave(file.path(
  fig_dir, "GRN_cnido_neuro_Pou4_FoxL2_jaccard.pdf"
), gp_jacc, height = 6, width = 8)
```
...
### Select peaks and genes in two retractor muscles
...
ggplot(gene_access, aes(gene_score)) +
  geom_histogram(bins = 100) +
  facet_wrap("cell_type") +
  scale_x_continuous(limits = c(0, 10), oob = squish, breaks = seq(0, 10, 1)) +
  labs(title = "gene scores per cell type") +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
    panel.grid.major.x = element_line(color = "grey", size = 0.2, linetype = "dotted"),
    panel.grid.major.y = element_line(color = "grey", size = 0.2, linetype = "dotted")
  )
ggsave(
  file.path(fig_dir, sprintf("fig4_gene_scores_hist_%s.pdf", ctn)),
  width = 10, height = 5
)
...
peak_gene_access_dt <- unique(peak_acc_dt[cell_type %in% cts])
peak_gene_access_dt[is.na(peak_access), peak_access := 0]
peak_gene_access_dt <- dcast.data.table(
  peak_gene_access_dt, peak + gene ~ cell_type, value.var = "peak_access", fill = 0
)

peak_gene_access_dt <- merge.data.table(
  peak_gene_access_dt, 
  unique(gene_access_dt[,.(gene,category)]) # all genes
  #unique(gene_access_dt[category != "other"][,.(gene,category)]) # all mmuscle acessible genes
  #unique(gene_access_dt[category == "both"][,.(gene,category)]) # MR and TR intersect genes
)
peak_access_dt <- unique(copy(peak_gene_access_dt)[, gene := NULL])
...
pdf(
  file.path(fig_dir, sprintf("fig4_peaks_access_%s_ovl_all_genes.pdf", ctn)),
  width = 4, height = 2
)
print(p1)
print(p2)
...
Promoters/enhancers among peaks in the intersect or specific for MR or TR

```{r}
# add promoter annotation to peak categories
peak_access_an_dt <- merge.data.table(
  unique(peak_access_dt[, .(peak, category)]),
  unique(peak_pro_dt[, .(peak, promoter)]),
  by = "peak", all.x = FALSE
)
...
peak_access_an_dt[promoter != "NO", promoter := "promoter"]
peak_access_an_dt[promoter == "NO", promoter := "enhancer"]
...
peak_access_an_sum <- peak_access_an_dt[,.N, .(category,promoter)][order(category,promoter)][, frac := N/sum(N), category]
peak_access_an_sum[, category := factor(
  category,
  levels = c(
    "muscle_tentacle_retractor",
    "muscle_mesentery_retractor",
...
ggplot(peak_access_an_sum, aes(x = "", y = frac, fill = promoter)) +
  ...
  theme_void() +
  theme(legend.position = "bottom")
ggsave(file.path(fig_dir, sprintf("fig4_peaks_promoters_enhancers_%s.pdf", ctn)), width = 6, height = 4)

```

How many genes that are shared ("both" category) don't share any peaks?

```{r}  
# all peaks categories per gene
gene_peak_cat_dt <- merge.data.table(
  peak_gene_access_dt[, .(gene, peak, gene_category=category)],
  peak_access_dt[, .(peak, peak_category=category)],
  by = "peak", all.x = TRUE
)
setorder(gene_peak_cat_dt, gene, peak)
gene_peak_sum_dt <- unique(
  unique(
    gene_peak_cat_dt[,.(gene,gene_category,peak_category)]
  )[,peak_categories := paste(sort(.SD$peak_category),collapse="+"),gene][, peak_category := NULL]
)

# only interested in genes in intersect between MR and TR
gene_peak_sum_dt <- gene_peak_sum_dt[gene_category=="both"]
gene_peak_sum_dt[,.N,peak_categories][order(-N)]

# do genes with shared accessibility also have shared peaks
gene_peak_sum_dt[, shared_peaks := "shared"]
gene_peak_sum_dt[peak_categories %in% c("muscle_tentacle_retractor", "muscle_mesentery_retractor", "muscle_mesentery_retractor+muscle_tentacle_retractor"), shared_peaks := peak_categories]
gene_peak_sum_dt[, shared_peaks := factor(
  shared_peaks,
  levels = c(
    "muscle_tentacle_retractor",
    "muscle_mesentery_retractor",
    "muscle_mesentery_retractor+muscle_tentacle_retractor",
    "shared"
  )
)]
gene_peak_sum_dt[, .N, shared_peaks][order(-N)][, frac := N/sum(N)][]

# pie chart
gp_sh <- ggplot(gene_peak_sum_dt, aes(x = "", fill = shared_peaks)) +
  geom_bar(width = 1, color = "white") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = c(
    ct_cols[cts],
    "muscle_mesentery_retractor+muscle_tentacle_retractor" = "#ffa200",
    "shared" = "#babab8"
  )) +
  theme_void() +
  theme(legend.position = "right")
ggsave(
  file.path(fig_dir, sprintf("fig4_shared_genes_shared_peaks_%s.pdf", ctn)),
  gp_sh,
  width = 6, height = 4
)

# promoter annotation
gene_peak_sum_pro_dt <- merge.data.table(
  gene_peak_sum_dt[gene_category=="both"][!grepl("other|both", peak_categories)], 
  unique(peak_pro_dt[promoter!="NO", .(gene,promoter)]), 
  by = "gene", all.x = TRUE
)
gene_peak_sum_pro_dt[,.N,.(peak_categories,promoter)][order(peak_categories,promoter)]

gs=unique(gene_peak_sum_pro_dt[peak_categories == "muscle_mesentery_retractor+muscle_tentacle_retractor"]$gene)
gs_mr=unique(gene_peak_sum_pro_dt[peak_categories == "muscle_mesentery_retractor"]$gene)
gs_tr=unique(gene_peak_sum_pro_dt[peak_categories == "muscle_tentacle_retractor"]$gene)

```

### Motif enrichment for peaks in overlap between retractor muscles

#### Calculate homer, streme and classic motif enrichment
...
Dotmap of motif enrichment for muscle-enriched motifs.

```{r}
# load general motif enrichment results
enr_dt <- rbindlist(lapply(c(
  #"results/Archetypes/motif-enrichment-cell-type-orthology-PPM-PCC-0.8-IC0.5-5bp-mona-q0.95.tsv.gz",
  "results/Archetypes/motif-enrichment-cell-type-archetypes-PPM-PCC-0.8-IC0.5-5bp-mona-q0.95.tsv.gz",
  "results/Motifs/motif-enrichment-cell-type-mona-q0.95.tsv.gz"
), function(x) fread(x)))

# select significantly enriched motifs in retractor muscles from general enrichment results
#top_mots <- unique(
#  enr_dt[grepl("retractor", cell_type) & padj < 0.01 & fc > 1.5]$motif
#)

# load significantly enriched motifs in retractor muscles specific peaks
msc_dt <- fread(
  file.path(mta_dir, "motif-enrichment-muscle-cell-types.tsv.gz")
)
msc_dt <- msc_dt[padj < 0.01 & fc > 1.5]
msc_dt[,.N,label]
top_mots <- unique(msc_dt$motif)
print(length(top_mots))

# select significantly enriched motifs in retractor muscles
top_mots_cov <- top_mots %in% unique(enr_dt$motif)
print(sprintf(
  "%s (%.2f%%) motifs in enrichment results; %s not found: %s",
  sum(top_mots_cov),
  mean(top_mots_cov)*100, 
  sum(!top_mots_cov),
  paste(sort(top_mots[!top_mots %in% unique(enr_dt$motif)]), collapse = ", ")
))
top_dt <- enr_dt[motif %in% top_mots]
top_dt <- merge.data.table(
  top_dt,
  msc_dt[, .(motif, group = label)],
  all.x = TRUE, allow.cartesian = TRUE
)[, .(group, motif, cell_type, fc, pval, padj)]
top_dt_mots <- unique(rbindlist(list(
  msc_dt[label=="TR"][, cell_type:="muscle_tentacle_retractor"],
  msc_dt[label=="MR"][, cell_type:="muscle_mesentery_retractor"],
  msc_dt[label=="both"][, cell_type:="muscle_mesentery_retractor"],
  msc_dt[label=="both"][, cell_type:="muscle_tentacle_retractor"]
))[, .(group=label, motif, cell_type, fc, pval, padj)])
unique(top_dt_mots[,.(motif,group)])[,.N,motif %in% top_dt$motif]
top_dt <- rbindlist(list(
  top_dt[! cell_type %in% c("muscle_mesentery_retractor", "muscle_tentacle_retractor")], 
  top_dt_mots
),  use.names = TRUE)
unique(top_dt[, .(motif, group)])[,.N,group]

# group motifs by name
top_dt[, motif_name := str_remove(motif, "ARCH\\d+_")]
top_dt[motif_name == "", motif_name := motif]
setorder(top_dt, -fc)
top_mots <- unique(top_dt[grep("retractor", cell_type), .SD[1], motif_name]$motif)
print(length(top_mots))
top_dt <- top_dt[motif %in% top_mots]

# motif to gene mapping
mot2gene <- fread(file.path(
    mta_dir, "motif-assignment-selected.tsv.gz"
))
mot2gene <- unique(mot2gene[, .(gene, gene_name, common_name, og, pfam, motif = archetype_name)])
mot2gene[common_name != "", gene_name := common_name]
map_to_genes <- FALSE
if (map_to_genes == TRUE) {
  top_dt <- merge.data.table(
    top_dt, mot2gene, by = "motif", all.x = TRUE, sort = FALSE, allow.cartesian = TRUE
  )
  top_dt[!is.na(gene_name), motif_name := gene_name]
  top_dt[motif_name == "", motif_name := motif]
  setorder(top_dt, -fc)
  top_mots <- unique(top_dt[grep("retractor", cell_type), .SD[1], motif_name]$motif)
  print(length(top_mots))
...
# order cell types by peak based tree
cell_types_tree <- c(
  "epidermis_2",
  "precursors_NPC",
  "muscle_tentacle_retractor",
...
top_dt[cell_type == "neuronal_gastrula", cell_type := "neuronal"]
stopifnot(all(top_dt$cell_type %in% cell_types_tree))
top_dt[, cell_type := factor(cell_type, levels = rev(cell_types_tree))]
setorder(top_dt, cell_type, -padj)

# manually fix motifs names
fn <- file.path(fig_dir, "fig4_muscle_motif_enrichment_names.tsv")
fn <- file.path(fig_dir, "fig4_muscle_motif_enrichment_names_shortlist.tsv")
if (file.exists(fn)) {
  # load file with nice motif names
  mot_nm <- fread(fn)
} else {
  # create motif names file if doesn't exist
  fwrite(
    unique(top_dt[, .(motif, motif_name, motif_name_clean = str_trunc(motif_name, 30))]),
    sep = "\t",
    fn
  )
}

# now add nice names as motif_name_clean column
if (!"motif" %in% colnames(mot_nm)) mot_nm[, motif := motif_name]
if (!"motif" %in% colnames(top_dt)) top_dt[, motif := motif_name]
if (!"motif_name" %in% colnames(top_dt)) top_dt[, motif_name := motif]

stopifnot(all(mot_nm$motif_name %in% top_dt$motif_name))

top_dt <- merge.data.table(
  mot_nm, top_dt, 
  by = c("motif_name", "group"), 
  all.x = TRUE, all.y = FALSE,
  sort = FALSE, 
  allow.cartesian = TRUE
)
top_dt[, motif_name := NULL]
setnames(top_dt, "nice_name", "motif_name")

# group motifs by name
setorder(top_dt, -fc)
top_mots <- unique(top_dt[, .SD[1], motif_name]$motif)
print(length(top_mots))
top_dt <- top_dt[motif %in% top_mots]

# motifs ordering
top_mt_ord <- c()
for (grp in c("TR", "MR", "both")) {
  top_dc <- dcast.data.table(
    unique(top_dt[group==grp][, .(motif_name, cell_type, padj)])[
      order(-padj), .SD[1], .(motif_name, cell_type)],
    motif_name ~ cell_type, value.var = "padj"
  )
  top_mt <- as.matrix(top_dc[, -1])
  rownames(top_mt) <- top_dc[[1]]
  top_mt[is.na(top_mt)] <- 0
  top_hc <- hclust(dist(top_mt))
  top_mt_ord <- c(top_mt_ord, rownames(top_mt)[top_hc$ord])
}

# order
plot_dt <- top_dt[motif_name %in% top_mt_ord]
plot_dt[, motif_name := factor(motif_name, levels = rev(top_mt_ord))]
setorder(plot_dt, -motif_name)

# plot values
plot_dt[, padj_scaled := -1 * log10(padj)]
plot_dt[, padj_scaled := pmin(padj_scaled, 20)]
plot_dt[, fc_scaled := pmin(fc, 8)]
plot_dt[padj_scaled == 0, padj_scaled:=NA]
plot_dt[, group := factor(group, levels = c("both", "MR", "TR"))]
setorder(plot_dt, group, motif_name, cell_type)

# plot
gp_mot_muscl <- ggplot(
    plot_dt,
    aes(motif_name, cell_type)
  ) +
  geom_point(
...
ggsave(
  file.path(fig_dir, "fig4_muscle_motif_enrichment_figure.pdf"),
  gp_mot_muscl,
  width = 8, height = 7
)

# save table with motifs and assigned genes for inspection
mot2gene[, genes := paste(unique(.SD$gene), collapse=";"), motif]
mot2gene[, gene_names := paste(unique(.SD$gene_name), collapse=";"), motif]
mot2gene[, common_names := paste(unique(.SD$common_name), collapse=";"), motif]
mot2gene[, ogs := paste(unique(.SD$og), collapse=";"), motif]
mot2gene[, pfams := paste(unique(.SD$pfam), collapse=";"), motif]
plot_an <- merge.data.table(
  plot_dt, 
  unique(mot2gene[, .(motif, genes, gene_names, common_names, ogs, pfams)]),
  by = "motif", all.x = TRUE, sort = FALSE
)
plot_an[is.na(plot_an)] <- ""
setcolorder(plot_an, c("group", "motif", "motif_name", "genes"))
fwrite(
  unique(plot_an[, .(group, motif_name, genes, gene_names, common_names, ogs, pfams)]),
  sep = "\t",
  file.path(fig_dir, "fig4_muscle_motif_enrichment_names_plot.tsv")
)
```
...
```{r}
arch_sim <- readRDS(file.path("results/Archetypes/motif-similarity-archetypes-PPM-PCC-0.8-IC0.5-5bp.rds"))
stopifnot(all(plot_an$motif %in% rownames(arch_sim)))
plot_sm <- plot_an[motif %in% rownames(arch_sim)]

for (x in c("MR", "TR", "both")) {

  x_dt <- unique(plot_sm[group == x, .(motif, motif_name)])
  x_sim <- arch_sim[x_dt$motif,x_dt$motif]
  rownames(x_sim) <- x_dt$motif_name
  colnames(x_sim) <- x_dt$motif_name
  ...
  # save heatmap
  hm_height <- pmax(dim(x_sim)[1]/4, 10)
  pdf(
    file.path(fig_dir, sprintf("fig4_muscle_ovl_%s_motif_similarity_heatmap.pdf", x)),
...
### Muscle TFs upstream regulators

From the global GRN, try to identify regulator of Nem64, Nem24 and Nem7 (i.e. TFs that might be hitting in their associated CREs). This could explain the mobilization of these paralogs in different germ layers (specially Nem64 in a neuroepithelial progenitor).

```{r}
# load global GRN
grn_dt <- readRDS("results/GRN/global_grn.rds")

# load insilico ChIP scores
isc_dt <- fread("results/insilicoChIP/insilico_chip_binding_score.tsv.gz")

# peak to TF assignment
pks_dt <- fread("results/Peaks/Peaks_cell_type_mapped_gene_assignment_coaccess.tsv.gz")
```

- Regulators of Nem TFs. We then will show these motifs in the Fig. 4 Nem landscapes.

```{r}
nem_pks <- list(
  "Nem64" = paste0("peak", 11051:11060),
  "Nem24" = paste0("peak", 11015:11019),
  "Nem7" = paste0("peak", 11019:11024)
)
nem_dt <- rbindlist(sapply(
  c("Nem64", "Nem24", "Nem7"),
  function(x) {
    isc_dt[peak %in% nem_pks[[x]]][
      in_silico_chip_score>0.05][
        order(peak, -in_silico_chip_score)][
          ,.(peak, motif, gene_name, common_name, og=stringr::str_trunc(og, 30), pfam=stringr::str_trunc(pfam, 30), in_silico_chip_score)][, in_grn := in_silico_chip_score>0.1] 
  },
  simplify = FALSE, USE.NAMES = TRUE
), idcol = "target_common_name")

fwrite(
  nem_dt,
  file.path(fig_dir, "fig4_nem_regulators.tsv"),
  sep = "\t"
)
```

- Regulators of tentacle muscle TFs.

```{r}
# get muscle TFs from GRN
tfs_fn <- "results/GRN/networks/cell_type/grn_tfs_info_cell_type_expression_fc_0.4_accessibility_access_0.4_chromvar_4.tsv.gz"
tfs_dt <- fread(tfs_fn)

tfs_ls <- list()
for (ct in c("muscle_mesentery_retractor", "muscle_tentacle_retractor")) {

  # get muscle TFs from GRN
  tfs_rt <- tfs_dt[cell_type == ct]$gene

  # get peaks of muscle TFs
  pks_rt <- pks_dt[gene %in% tfs_rt]$peak

  # get in silico chip scores in selected muscle peaks
  isc_rt <- isc_dt[peak %in% pks_rt][in_silico_chip_score>0.1]

  # count hits of any given TF in selected peaks
  isc_rt[, n_hits := .N, gene]
  tfs_rt <- unique(isc_rt[order(-n_hits)])
  tfs_ls[[ct]] <- tfs_rt[, .(gene, pfam, og, gene_name, common_name, n_hits)]

}

# save
tfs_rt <- unique(rbindlist(tfs_ls, idcol = "cell_type"))
fwrite(
  tfs_rt, 
  file.path(fig_dir, "fig4_muscle_tfs_regulators.tsv"),
  sep = "\t"
)


tfs_rt_dc <- dcast.data.table(tfs_rt, gene+pfam+og+gene_name+common_name~cell_type, value.var="n_hits")
tfs_rt_gp <- ggplot(tfs_rt_dc, aes(aes(muscle_mesentery_retractor, muscle_tentacle_retractor))) +
  geom_point() +
  ggrepel::geom_text_repel() +
  coord_fixed()
ggsave(
  file.path(fig_dir, "fig4_muscle_tfs_regulators.pdf"),
  tfs_rt_gp, widt = 8, height = 8
)
```

- accessibilirty of TFs in retractor muscles

```{r}
# get genes accessiblity
acc_fn <- "results/GeneScoreMatrix/Matrix-Gene-Scores-cell-type-FC.rds"
acc_mt <- readRDS(acc_fn)
acc_tf <- acc_mt[
  intersect(rownames(acc_mt), tfan$gene), 
  c("muscle_tentacle_retractor", "muscle_mesentery_retractor")
]
acc_dt <- as.data.table(as.matrix(acc_tf), keep.rownames = "gene")
acc_dt <- merge.data.table(acc_dt, tfan, by = "gene", all.x = TRUE)
acc_dt[, label := common_name]
acc_dt[label == "", label := substr(gene_name, 1, 15)]
acc_dt[label == "", label := substr(og, 1, 15)]
acc_dt[label == "", label := substr(pfam, 1, 15)]
acc_dt[label == "", label := gene]
thrs <- 3
acc_dt[muscle_tentacle_retractor < thrs & muscle_mesentery_retractor < thrs, label := ""]
acc_dt[, set := "other"]
acc_dt[muscle_tentacle_retractor > thrs & muscle_mesentery_retractor < thrs + 1, set := "muscle_tentacle_retractor"]
acc_dt[muscle_tentacle_retractor < thrs + 1 & muscle_mesentery_retractor > thrs, set := "muscle_mesentery_retractor"]

# scatterplot
acc_gp <- ggplot(acc_dt, aes(muscle_tentacle_retractor, muscle_mesentery_retractor)) +
  ...
  scale_color_manual(values = c(ct_cols, "other" = "gray50")) +
  coord_fixed() +
  labs(
    x = "TR accessibility",
    y = "MR accessibility"
...
ggsave(
  file.path(fig_dir, "fig4_accessibility.pdf"),
  acc_gp,
  height = 5, width = 5
)
