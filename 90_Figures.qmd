...
peak_gene_access_dt <- merge.data.table(
  ...
  #unique(gene_access_dt[category != "other"][,.(gene,category)]) # acessible genes
  unique(gene_access_dt[category == "both"][,.(gene,category)]) # intersect genes
)
...
peak_access_dt[peak_access_dt[[cts[1]]] > 4 & peak_access_dt[[cts[2]]] > 4, category := "both"]
...
pdf(
  file.path(fig_dir, sprintf("fig4_peaks_access_%s_ovl_intersect_genes.pdf", ctn)),
  width = 4, height = 2
)
print(p1)
print(p2)
...
ggplot(peak_access_an_sum, aes(x = "", y = frac, fill = promoter)) +
  ...
  theme_void() +
  theme(legend.position = "bottom")
ggsave(file.path(fig_dir, sprintf("fig4_peaks_promoters_enhancers_%s_accessible.pdf", ctn)), width = 6, height = 4)
...
gene_peak_sum_dt[, exclusive_peak := FALSE]
gene_peak_sum_dt[peak_categories %in% c("muscle_tentacle_retractor", "muscle_mesentery_retractor"), exclusive_peak := TRUE]
gene_peak_sum_dt[, .N, exclusive_peak][order(-N)]
...
#)
#top_mots <- unique(msc_dt[padj < 0.01 & fc > 1.5]$motif)
...
mot2gene <- unique(mot2gene[, .(gene, gene_name, common_name, motif = archetype_name)])
...
stopifnot(all(top_dt$cell_type %in% cell_types))
top_dt[, cell_type := factor(cell_type, levels = cell_types)]
setorder(top_dt, cell_type, -padj)

# manually fix motifs name
if (file.exists(file.path(fig_dir, "fig4_muscle_motif_enrichment_names.tsv"))) {
  ...
  fwrite(
    unique(top_dt[, .(motif, motif_name)]),
    sep = "\t",
    file.path(fig_dir, "fig4_muscle_motif_enrichment_names.tsv")
  )
}
...
# motifs ordering
top_dc <- dcast.data.table(
  unique(top_dt[, .(motif_name, cell_type, padj)])[
...
rownames(top_mt) <- top_dc[[1]]
top_mt[is.na(top_mt)] <- 0
# order motifs by max enrich cell type
#top_co <- apply(top_mt, 1, which.min)
#top_mt_ord <- unlist(sapply(
#  unique(top_co), function(x) {
#    m <- top_mt[top_co == x, x]
...
top_mt_ord <- rownames(top_mt)[top_hc$ord]
