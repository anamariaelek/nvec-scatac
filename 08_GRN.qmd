...
bct_maps[cell_type == "neuron_Pou4_FoxL2_3", broad_cell_type := "neuron_Pou4_FoxL2_3"]
bct_maps[cell_type == "gland_mucin", broad_cell_type := "gland_mucin"]
bct_cols <- c(bct_cols, ct_cols)[unique(bct_maps$broad_cell_type)]
...
thrs <- 0.1
grn_dt <- grn_all_dt[in_silico_chip_score > thrs]
...
grn_filt_dt[is.na(target_gene_name), target_gene_name := ""]
grn_filt_dt[is.na(target_common_name), target_common_name := ""]
grn_filt_dt[is.na(target_og), target_og := ""]
grn_filt_dt[is.na(target_pfam), target_pfam := ""]
...
# group all target genes by number of TF inputs
...
# number of peaks per TF target gene
dt_tg_tfs_pks <- unique(
  grn_dt[!motif %in% "ARCH895_Foxn4/Foxn1"][, .(gene, peak, target_gene)]
)
tfs_target_gene <- unique(dt_tg_tfs_pks[gene == target_gene]$gene)
dt_tg_tfs_pks[, target_TF := gene %in% tfs_target_gene]
dt_tg_tfs_pks[, n_tf_peaks := length(unique(.SD$peak)), .(gene, target_gene, target_TF)]
dt_tg_tfs_pks[, n_peaks := length(unique(.SD$peak)), .(target_gene, target_TF)]
dt_tg_tfs_pks[, n_tfs := length(unique(.SD$gene)), .(target_gene, target_TF)]
dt_tg_tfs_pks[, peak := NULL]
dt_tg_tfs_pks <- unique(dt_tg_tfs_pks)

# annotation for plot
dt_tf_tfs_ann <- merge.data.table(
  unique(dt_tg_tfs_pks[, n_tfs:=.N, .(n_peaks, n_tf_peaks)][n_tfs < 3]), 
  unique(
    grn_dt[, .(gene, gene_name, common_name, og, pfam, target_gene, target_gene_name, target_common_name, target_og, target_pfam)]
  ),
  by = c("gene", "target_gene"), all.x = TRUE, sort = FALSE
)
dt_tf_tfs_ann[common_name != "", gene_name := common_name]
dt_tf_tfs_ann[target_common_name != "", target_gene_name := target_common_name]
dt_tf_tfs_ann[gene_name == "", gene_name := stringr::str_trunc(og, 25)]
dt_tf_tfs_ann[target_gene_name == "", target_gene_name := stringr::str_trunc(target_og, 25)]
dt_tf_tfs_ann[, label := sprintf("%s -> %s", gene_name, target_gene_name)]

# plot
gp_tg_tfs_pks <- ggplot(
    dt_tg_tfs_pks[, .N, .(n_peaks, n_tf_peaks)], 
    aes(n_peaks, n_tf_peaks)
  ) +
  geom_text_repel(
    data = dt_tf_tfs_ann[n_peaks > 12],
    aes(label = label),
    nudge_x = Inf,
    nudge_y = 0.5,
  ...
  geom_point(aes(size = N), color = "grey", alpha = 0.5) +
...
ggsave(
  file.path(fig_dir, "grn-global-num-tfs-targets-pks.pdf"),
  gp_tg_tfs_pks, height = 14, width = 14
)
    

# group all target genes by number of TF inputs
dt_tg_tfs_pks[, Nchr := as.character(pmin(n_tf_peaks, 11))]
dt_tg_tfs_pks[Nchr=="11", Nchr := ">10"]
dt_tg_tfs_pks[, Nchr := factor(Nchr, levels = c(as.character(0:10), ">10"))]

# plot
gp_tg_tfs_pks <- ggplot(dt_tg_tfs_pks, aes(Nchr)) +
    geom_bar(fill = "#3d3d3d") +
    geom_text(
      data = dt_tg_tfs_pks[,.(md=median(n_peaks))],
      aes(x = "6", y = 5e3, label = sprintf("median:\n%s", md)),
      size = 5
    ) +
    scale_y_continuous(
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    labs(y = "target genes", x = "peaks per\nTF-target gene")



# save plot
gp_grn <- gp_tf_gen + gp_tg_tfs + gp_tg_tfs_pks +
  plot_layout(nrow = 1, ncol = 3, guides = "collect", width = c(2, 3, 3))
ggsave(
  file.path(fig_dir, "grn-global-num-tfs-targets.pdf"),
  gp_grn, height = 4, width = 10
)
```

Inspect overlap of predicted targets with ChIP data.

```{r}
# load brachiury peaks
bra_ls <- lapply(1:2, function(x) {

  bra_dt <- fread(sprintf("data/Bra_%s_macs_narrow_peaks.narrowPeak", x))
  setnames(bra_dt, c("seqnames", "start", "end", "peak", "score", "strand", "signalValue", "pValue", "qValue", "summit"))

  # reduce overlapping peaks
  bra_gr <- makeGRangesFromDataFrame(bra_dt, keep.extra.columns = TRUE)
  bra_gr <- reduce(bra_gr)
  bra_gr

})

# overlap brachiury peaks in two samples
ovl <- GenomicRanges::findOverlaps(query = bra_ls[[1]], subject = bra_ls[[2]], type = "any", minoverlap = 10, ignore.strand = TRUE)
bra_gr_1 <- bra_ls[[1]][queryHits(ovl)]
bra_gr_2 <- bra_ls[[2]][subjectHits(ovl)]
bra_gr <- sort(c(bra_gr_1, bra_gr_2))
bra_gr <- reduce(bra_gr)
bra_gr$peak <- paste0("Bra_", seq_along(bra_gr))

# get brachiury targets from global GRN
out_list <- list()
thrs <- seq(0, 0.1, 0.01)
for (i in seq_along(thrs)) {

  insilico_chip_thrs <- thrs[i]
  bra_tg <- unique(grn_all_dt[common_name == "Brachyury"][in_silico_chip_score > insilico_chip_thrs])
  bra_tg <- makeGRangesFromDataFrame(bra_tg, keep.extra.columns = TRUE)

  # overlap
  ovl <- GenomicRanges::findOverlaps(query = bra_gr, subject = bra_tg, type = "any", minoverlap = 10, ignore.strand = TRUE)
  bra_gr_ovl <- bra_gr[queryHits(ovl)]
  bra_tg_ovl <- bra_tg[subjectHits(ovl)]

  # euler
  require(eulerr)
  ovl_list <- list(
    ChIP = unique(c(
      # ChIP peaks ovrlapping GRN targets
      bra_tg_ovl$peak,
      # ChIP peaks not overlapping GRN targets
      setdiff(bra_gr$peak, bra_gr_ovl$peak))),
    # all targets
    GRN = unique(bra_tg$peak)
  )
  eul <- euler(ovl_list)
  gp_eul <- plot(eul, quantities = TRUE)
  #ggsave(
  #  file.path(fig_dir, sprintf("grn-global-brachyury-targets-%s-euler.pdf", insilico_chip_thrs)),
  #  gp_eul, height = 5, width = 5
  #)

  # save
  out_list[[i]] <- data.table(
    insilico_chip_threshold = insilico_chip_thrs,
    chip_pekas = length(ovl_list$ChIP),
    grn_targets = length(ovl_list$GRN),
    grn_in_chip = length(unique(bra_tg_ovl$peak))
  )
}

ovl_dt <- rbindlist(out_list)
ovl_dt[, fraction_grn_in_chip := grn_in_chip / grn_targets]
ovl_dt[, fraction_chip_in_grn := grn_in_chip / chip_pekas]

# transform for plotting 
ovl_dt <- melt.data.table(ovl_dt, id.vars = "insilico_chip_threshold")

gp_ovl <- ggplot(ovl_dt[variable != "chip_pekas"], aes(insilico_chip_threshold, value)) +
  geom_bar(stat = "identity") +
  facet_wrap(~variable, scales = "free_y", ncol = 1) +
  theme(panel.grid.major = element_line())

ggsave(
  file.path(fig_dir, "grn-global-brachyury-targets-overlap.pdf"),
  gp_ovl, height = 10, width = 6
)
```
...
if (lvl == "broad_cell_type") {
  stopifnot(all(acc_dt$cell_type %in% bct_maps$cell_type))
  acc_dt <- merge.data.table(
      acc_dt, bct_maps, by = "cell_type",
      all.x = TRUE, sort = FALSE
  )
  acc_dt[, cell_type := broad_cell_type]
  acc_dt[, broad_cell_type := NULL]
}
...
} else if (lvl == "broad_cell_type") {
  stopifnot(all(acc_dt$cell_type %in% broad_cell_types))
}
...
if (lvl == "broad_cell_type") {
  stopifnot(all(exp_umi$cell_type %in% bct_maps$cell_type))
  exp_umi <- merge.data.table(
      exp_umi, bct_maps, by = "cell_type",
      all.x = TRUE, sort = FALSE
  )
  exp_umi[, cell_type := broad_cell_type]
  exp_umi[, broad_cell_type := NULL]
  exp_umi <- exp_umi[, .(expression_umifrac = mean(expression_umifrac)), .(gene, cell_type)]
}
...
if (lvl == "broad_cell_type") {
  stopifnot(all(exp_fc$cell_type %in% bct_maps$cell_type))
  exp_fc <- merge.data.table(
      exp_fc, bct_maps, by = "cell_type",
      all.x = TRUE, sort = FALSE
  )
  exp_fc[, cell_type := broad_cell_type]
  exp_fc[, broad_cell_type := NULL]
  exp_fc <- exp_fc[, .(expression_fc = mean(expression_fc)), .(gene, cell_type)]
}
...
if (lvl == "cell_type") exp_dt[, stage := ifelse(cell_type %in% adult_cell_types, "adult", "gastrula")]
...
if (lvl == "broad_cell_type") {
  stopifnot(all(gsc_dt$cell_type %in% bct_maps$cell_type))
  gsc_dt <- merge.data.table(
      gsc_dt, bct_maps, by = "cell_type",
      all.x = TRUE, sort = FALSE
  )
  gsc_dt[, cell_type := broad_cell_type]
  gsc_dt[, broad_cell_type := NULL]
  gsc_dt <- gsc_dt[, .(gene_score = mean(gene_score)), .(gene, cell_type)]
}
if (lvl == "cell_type") gsc_dt[, stage := ifelse(cell_type %in% adult_cell_types, "adult", "gastrula")]

# combine gene scores with expression
dist_dt <- merge.data.table(
    exp_dt, gsc_dt, by = c("gene", "cell_type"), 
    sort = FALSE
)

# combine peak accessibility with expression and gene scores
dist_dt <- merge.data.table(
    dist_dt, acc_dt, by = c("gene", "cell_type"),
    all.x = TRUE, sort = FALSE
)
...
gns_exp_acc_list <- sapply(unname(genes), function(g) {
    ...
    gns_exp <- merge.data.table(
        ...
        unique(dist_dt[gene == g, .(
            gene, cell_type, expression_umifrac, expression_fc, gene_score
        )]),
        by = c("gene"),
    ...
    gns_ext <- merge.data.table(
        gns_exp, tgs_dt,
        by = c("target_gene", "peak", "cell_type"),
...
saveRDS(
    gns_exp_acc_list,
    file.path(
        grn_dir, sprintf("grn_%s_tf_targets_expression_accessibility.rds", lvl)
    )
)
```
...
# level
lvl <- "broad_cell_type"
cell_type_lvl <- switch(lvl, "broad_cell_type" = broad_cell_types, "cell_type" = cell_types)
...
if (is.null(thr_fc_exp)) {
  fn <- sprintf(
    "%s_expression_%s_%s_accessibility_%s_%s_chromvar_%s", 
    lvl, exp, thr_q_exp, acc, thr_q_acc, chr_thrs
  )
} else {
  fn <- sprintf(
    "%s_expression_%s_%s_accessibility_%s_%s_chromvar_%s", 
    lvl, exp, thr_fc_exp, acc, thr_q_acc, chr_thrs
  )
}

# results dir
grn_res_dir <- file.path(grn_dir, "networks", lvl)
...
if (lvl == "broad_cell_type") {
  stopifnot(all(devel_dt$cell_type %in% bct_maps$cell_type))
  devel_dt <- merge.data.table(
      devel_dt, bct_maps, by = "cell_type",
      all.x = TRUE, sort = FALSE
  )
  devel_dt[, cell_type := broad_cell_type]
  devel_dt[, broad_cell_type := NULL]
}
```
...
if (lvl == "broad_cell_type") {
  stopifnot(all(exp_chromvar_dt$cell_type %in% bct_maps$cell_type))
  exp_chromvar_dt <- merge.data.table(
      exp_chromvar_dt, bct_maps, by = "cell_type",
      all.x = TRUE, sort = FALSE
  )
  exp_chromvar_dt[, cell_type := broad_cell_type]
  exp_chromvar_dt[, broad_cell_type := NULL]
}

# aggregate zscores per cell type
if ("archetype_name" %in% colnames(exp_chromvar_dt)) setnames(exp_chromvar_dt, "archetype_name", "motif")
if ("motif_deviation" %in% colnames(exp_chromvar_dt)) setnames(exp_chromvar_dt, "motif_deviation", "zscore")
...
if (lvl == "cell_type") {
  ...
  stopifnot(all(exp_chromvar_dt$cell_type %in% broad_cell_types))
  exp_chromvar_dt[, cell_type := factor(cell_type, levels = broad_cell_types)]
}

fwrite(
  exp_chromvar_dt, 
  file.path(grn_dir, sprintf(
    "chromVAR_%s.tsv.gz", lvl
  )), sep = "\t")
```
...
gns_exp_acc_list <- readRDS(
    file.path(
        grn_dir,
        sprintf("grn_%s_tf_targets_expression_accessibility.rds", lvl)
    )
)
...
if ("stage" %in% colnames(gns_exp_acc_act_dt)) gns_exp_acc_act_dt[, stage := NULL]
if (lvl == "cell_type") {
  stopifnot(all(gns_exp_acc_act_dt$cell_type %in% cell_types))
  gns_exp_acc_act_dt[, cell_type := factor(cell_type, levels = cell_types)]
} else if (lvl == "broad_cell_type") {
  stopifnot(all(gns_exp_acc_act_dt$cell_type %in% broad_cell_types))
  gns_exp_acc_act_dt[, cell_type := factor(cell_type, levels = broad_cell_types)]
}
setorder(gns_exp_acc_act_dt, cell_type, gene, target_gene)
```

Save unfiltered GRNs per cell type.

```{r}
require(openxlsx)
#wb_peaks <- createWorkbook()
wb_genes <- createWorkbook()
for (ct in cell_type_lvl) {
  sheetName <- substr(ct, 1, 30)
  # global network in selected cell type
  grn_ct <- gns_exp_acc_act_dt[cell_type == ct]
  #addWorksheet(wb_peaks, sheetName = sheetName)
  #writeData(wb_peaks, sheet = sheetName, grn_ct)
  # summarize per gene
  grn_ct[, c("target_peak", "seqnames", "start", "end") := NULL]
  grn_ct[, target_accessibility := max(target_accessibility), .(gene, target_gene, cell_type)]
  grn_ct[, peak_tf_correlation_score := max(peak_tf_correlation_score), .(gene, target_gene, cell_type)]
  grn_ct[, in_silico_chip_score := max(in_silico_chip_score), .(gene, target_gene, cell_type)]
  grn_ct <- unique(grn_ct)
  # add to file
  addWorksheet(wb_genes, sheetName = sheetName)
  writeData(wb_genes, sheet = sheetName, grn_ct)
}
#saveWorkbook(
#  wb_peaks,
#  file.path(grn_res_dir, sprintf("grn_global_peaks.xlsx")),
#  overwrite = TRUE
#)
saveWorkbook(
  wb_genes,
  file.path(grn_res_dir, sprintf("grn_global_genes.xlsx")),
  overwrite = TRUE
)
```
...
stopifnot(all(data_thrs$cell_type %in% cell_type_lvl))
data_thrs[, cell_type := factor(cell_type, levels = cell_type_lvl)]
...
grn_base_dt <- merge.data.table(
  gns_exp_acc_act_dt, data_thrs[, .(cell_type, exp_thrs, acc_thrs)], 
  by = c("cell_type")
)
```
...
tfs_info_dt <- unique(grn_filt_dt[, .(
  cell_type, gene, gene_name, common_name, og, pfam, 
...
fwrite(
  tfs_info_dt,
  file.path(
    grn_res_dir, sprintf("grn_tfs_info_%s.tsv.gz", fn)
  ),
  sep = "\t"
)
require(openxlsx)
...
saveWorkbook(
  wb,
  file.path(grn_res_dir, sprintf("grn_tfs_info_%s.xlsx", fn)),
  overwrite = TRUE
)
```
...
tfs_info_dt[, cell_type_gene := paste(cell_type, gene)]
grn_filt_dt[, cell_type_gene := paste(cell_type, gene)]
grn_fill_dt <- rbindlist(list(
  grn_filt_dt,
  tfs_info_dt[! cell_type_gene %in% grn_filt_dt$cell_type_gene]
), use.names = TRUE, fill = TRUE)
grn_fill_dt[, exp_thrs := .SD[!is.na(exp_thrs)]$exp_thrs[1], .(cell_type)]
grn_fill_dt[, acc_thrs := .SD[!is.na(acc_thrs)]$acc_thrs[1], .(cell_type)]
grn_fill_dt[, cell_type_gene := NULL]
tfs_info_dt[, cell_type_gene := NULL]
setorder(grn_fill_dt, cell_type)

# target TF annotations
grn_fill_dt[, target_TF := target_gene %in% tfan$gene]
grn_fill_dt[, target_active_TF := target_gene %in% .SD$gene, .(cell_type)]

# save
fwrite(
  grn_fill_dt,
  file.path(
    grn_res_dir, sprintf("grn_peaks_%s.tsv.gz", fn)
  ),
  sep = "\t"
)
require(openxlsx)
...
saveWorkbook(
  wb,
  file.path(grn_res_dir, sprintf("grn_peaks_%s.xlsx", fn)),
  overwrite = TRUE
)
```
...
grn_gene_dt[, target_accessibility := max(target_accessibility), .(gene, target_gene, cell_type)]
grn_gene_dt[, peak_tf_correlation_score := max(peak_tf_correlation_score), .(gene, target_gene, cell_type)]
grn_gene_dt[, in_silico_chip_score := max(in_silico_chip_score), .(gene, target_gene, cell_type)]
...
fwrite(
  grn_gene_dt,
  file.path(
    grn_res_dir, sprintf("grn_genes_%s.tsv.gz", fn)
  ),
  sep = "\t"
)
require(openxlsx)
...
saveWorkbook(
  wb,
  file.path(grn_res_dir, sprintf("grn_genes_%s.xlsx", fn)),
  overwrite = TRUE
)
```
...
# any TFs that have no target genes
tfs_info_dt[, cell_type_gene := paste(cell_type, gene)]
grn_tfs_dt[, cell_type_gene := paste(cell_type, gene)]
grn_tfs_dt <- rbindlist(list(
  grn_tfs_dt,
  tfs_info_dt[! cell_type_gene %in% grn_tfs_dt$cell_type_gene]
), use.names = TRUE, fill = TRUE)
grn_tfs_dt[, exp_thrs := .SD[!is.na(exp_thrs)]$exp_thrs[1], .(cell_type)]
grn_tfs_dt[, acc_thrs := .SD[!is.na(acc_thrs)]$acc_thrs[1], .(cell_type)]
grn_tfs_dt[, cell_type_gene := NULL]
tfs_info_dt[, cell_type_gene := NULL]
setorder(grn_tfs_dt, cell_type)

# save
fwrite(
  grn_tfs_dt,
  file.path(
    grn_res_dir, sprintf("grn_tfs_%s.tsv.gz", fn)
  ),
  sep = "\t"
)
require(openxlsx)
...
saveWorkbook(
  wb,
  file.path(grn_res_dir, sprintf("grn_tfs_%s.xlsx", fn)),
  overwrite = TRUE
)
```
...
stopifnot(all(grn_ct_dt$cell_type %in% cell_type_lvl))
grn_ct_dt[, cell_type := factor(cell_type, levels = cell_type_lvl)]

# gene expression
dist_dt <- fread(file.path(
  map_dir, lvl, "data_expression_accessibility_genes_exp_FC2_acc_FC4_spearman.tsv.gz"
))
expr_dt <- unique(dist_dt[, .(gene, cell_type, expression_fc, expression_umifrac, gene_score)])

# expression thresholds used for GRN filtering
data_thrs <- fread(file.path(
    grn_dir, sprintf(
        "qthreshold_%s_expression_%s_accessibility_%s.tsv",
        lvl, thr_q_exp, thr_q_acc
...
expr_dt <- merge.data.table(expr_dt, data_thrs, by = "cell_type", all.x = TRUE)
expr_filt <- expr_dt[expression_fc > thr_exp_fc]
accs_filt <- expr_dt[gene_score > thr_gene_score+0.01]
...
chrv_filt <- exp_chromvar_dt[, .(gene, cell_type, zscore)][zscore > chr_thrs]
```
...
cts_cols <- switch(lvl, "broad_cell_type" = bct_cols, "cell_type" = ct_cols)

# gene with a bad motif
bckl <- "Nvec_vc1.1_XM_032362607.2"
grn_ct_dt <- grn_ct_dt[! gene %in% bckl]
...
cp_tf_num <- ggplot(
    ...
    scale_fill_manual(values = cts_cols, drop = TRUE) +
    scale_color_manual(values = structure(
      colorspace::darken(cts_cols, 0.5),
      names = names(cts_cols)
    )) +
...
ct_tf_tgs <- grn_ct_dt[, n_targets := .N, .(gene, cell_type)]
ct_tf_tgs[, cell_type_box := paste0(cell_type, "_box")]
ct_tf_tgs[,ntfs:=length(unique(.SD$gene)),cell_type]
cp_tf_tgs <- ggplot(
        ct_tf_tgs, aes(
          cell_type, n_targets,
          color = cell_type
    ...
    scale_fill_manual(values = c(
      cts_cols, structure(
        cts_cols,
        names = paste0(names(cts_cols), "_box")
    )), guide = "none") +
    scale_color_manual(values = structure(
      colorspace::darken(cts_cols, 0.5),
      names = names(cts_cols)
...
ct_tf_ovl[, cell_type := factor(cell_type, levels = cell_type_lvl)]
ct_tf_ovl[, cell_type_box := paste0(cell_type, "_box")]
ct_tf_ovl[, mean(target_overlap), cell_type]
ct_tf_ovl[,n:=nrow(.SD[target_overlap>0]),cell_type]
cp_tg_ovl <- ggplot(
        ct_tf_ovl, aes(
...
# plot
gp_grn_ct <- cp_tf_num / cp_tf_tgs / cp_tg_ovl
gp_grn_ct <- gp_grn_ct +
  plot_annotation(title = sprintf(
      "Expression %s %s & chromVAR Z %s",
      exp, sprintf("quantile %s", thr_q_exp), chr_thrs
  ))

# save
ggsave(
  file.path(fig_dir, sprintf("grn_num_tfs_targets_%s.pdf", fn)), 
  gp_grn_ct, 
  height = 10, width = 6
)
...
Number of TFs vs number of expressed genes per cell type

```{r}
expr_ct_dt <- merge.data.table(
  expr_filt[, .(n_expr_genes=.N), cell_type],
  expr_filt[gene %in% tfan$gene][, .(n_expr_tfs=.N), cell_type],
  by = "cell_type", all.x = TRUE
)

accs_ct_dt <- merge.data.table(
  accs_filt[, .(n_accs_genes=.N), cell_type],
  accs_filt[gene %in% tfan$gene][, .(n_accs_tfs=.N), cell_type],
  by = "cell_type", all.x = TRUE
)

actv_ct_dt <- unique(grn_ct_dt[, .(cell_type, gene)])[, .(n_active_tfs=.N), cell_type]

chrv_ct_dt <- chrv_filt[, .(n_chrv_genes=.N), cell_type]

n_tf_genes_dt <- merge.data.table(
  expr_ct_dt, accs_ct_dt,
  by = "cell_type", all.x = TRUE
)
n_tf_genes_dt <- merge.data.table(
  n_tf_genes_dt, actv_ct_dt,
  by = "cell_type", all.x = TRUE
)
n_tf_genes_dt <- merge.data.table(
  n_tf_genes_dt, chrv_ct_dt,
  by = "cell_type", all.x = TRUE
)

# EXPRESSION
cor_gns <- cor.test(n_tf_genes_dt$n_active_tfs, n_tf_genes_dt$n_expr_genes)
cp_tf_gen_num <- ggplot(
        n_tf_genes_dt,
        aes(n_expr_genes, n_active_tfs, fill = cell_type, color = cell_type)
    ) +
    ...
    annotate(
      ...
      label = sprintf(
        "r = %.2f, p = %.2e", 
        cor_gns$estimate, cor_gns$p.value
      ),
      size = 4, color = "black"
    ) + 
    scale_fill_manual(values = cts_cols, drop = TRUE) +
    ...
    labs(x = "# expressed genes", y = "# active TFs")

cor_tfs <- cor.test(n_tf_genes_dt$n_active_tfs, n_tf_genes_dt$n_expr_tfs)
cp_tf_tf_num <- ggplot(
        n_tf_genes_dt,
        aes(n_expr_tfs, n_active_tfs, fill = cell_type, color = cell_type)
    ) +
    geom_point(shape = 21, size = 2) +
    geom_smooth(
      aes(group = 1),
    ...
    annotate(
      ...
      label = sprintf(
        "r = %.2f, p = %.2e", 
        cor_tfs$estimate, cor_tfs$p.value
      ),
      size = 4, color = "black"
    ) +
    scale_fill_manual(values = cts_cols, drop = TRUE) +
    ...
    labs(x = "# expressed TFs", y = "# active TFs")



# ACCESSIBILITY
cor_accs_gns <- cor.test(n_tf_genes_dt$n_active_tfs, n_tf_genes_dt$n_accs_genes)
cp_tf_gen_accs_num <- ggplot(
        n_tf_genes_dt,
        aes(n_accs_genes, n_active_tfs, fill = cell_type, color = cell_type)
    ) +
    geom_point(shape = 21, size = 2) +
    geom_smooth(
      aes(group = 1),
    ...
    annotate(
      ...
      label = sprintf(
        "r = %.2f, p = %.2e", 
        cor_accs_gns$estimate, cor_accs_gns$p.value
      ),
      size = 4, color = "black"
    ) +
    scale_fill_manual(values = cts_cols, drop = TRUE) +
    ...
    labs(x = "# accessible genes", y = "# active TFs")

cor_accs_tfs <- cor.test(n_tf_genes_dt$n_active_tfs, n_tf_genes_dt$n_accs_tfs)
cp_tf_tf_accs_num <- ggplot(
        n_tf_genes_dt,
        aes(n_accs_tfs, n_active_tfs, fill = cell_type, color = cell_type)
    ) +
    geom_point(shape = 21, size = 2) +
    geom_smooth(
      aes(group = 1),
    ...
    annotate(
      ...
      label = sprintf(
        "r = %.2f, p = %.2e", 
        cor_accs_tfs$estimate, cor_accs_tfs$p.value
      ),
      size = 4, color = "black"
    ) +
    scale_fill_manual(values = cts_cols, drop = TRUE) +
    ...
    labs(x = "# accessible TFs", y = "# active TFs")


# CHROMVAR
cor_chrv_gns <- cor.test(n_tf_genes_dt$n_active_tfs, n_tf_genes_dt$n_chrv_genes)
cp_tf_chrv_num <- ggplot(
        n_tf_genes_dt,
        aes(n_chrv_genes, n_active_tfs, fill = cell_type, color = cell_type)
    ) +
    geom_point(shape = 21, size = 2) +
    geom_smooth(
      aes(group = 1),
    ...
    annotate(
      ...
      label = sprintf(
        "r = %.2f, p = %.2e", 
        cor_chrv_gns$estimate, cor_chrv_gns$p.value
      ),
      size = 4, color = "black"
    ) +
    scale_fill_manual(values = cts_cols, drop = TRUE) +
    ...
    labs(x = "# chromVAR TFs", y = "# active TFs")

ggsave(
  file.path(fig_dir, sprintf("grn_num_tfs_num_genes_%s.pdf", fn)), 
  (plot_spacer() + cp_tf_chrv_num) / (cp_tf_gen_num + cp_tf_tf_num) / (cp_tf_gen_accs_num + cp_tf_tf_accs_num), 
  height = 12, width = 8
)

```

Network metrics and cartography

```{r}
source("motif-analysis/mta_grn_functions.R")

# this is a global expression threshold used for plotting the networks
# it overrides expression quantification and quantile threshold
thr_fc_exp <- 1.2

if (!is.null(thr_fc_exp)) {
  grn_net_dt <- grn_ct_dt[expression_fc > thr_fc_exp]
  fn <- sprintf(
    "%s_expression_%s_%s_accessibility_%s_%s_chromvar_%s", 
    lvl, exp, thr_fc_exp, acc, thr_q_acc, chr_thrs
  )
} else {
  grn_net_dt <- copy(grn_ct_dt)
}

# calculate network metrics
cart_list <- lapply(cell_type_lvl, function(ct) {
  message("Cartography for ", ct, " GRN")
  grn_ct <- unique(grn_net_dt[cell_type == ct][, .(gene, target_gene, in_silico_chip_score)])
  tryCatch({
    mta_network_scores(coef_dt = grn_ct)[, cell_type := ct]
  }, error = function(e) {
    message("Error in ", ct, ": ", e)
    NULL
  })
})
cart_dt <- rbindlist(cart_list, use.names = TRUE, fill = TRUE)
stopifnot(all(cart_dt$cell_type %in% cell_type_lvl))
cart_dt[, cell_type := factor(cell_type, levels = cell_type_lvl)]

# save
fwrite(cart_dt, file.path(
  grn_res_dir, sprintf(
    "cartography_%s.tsv", fn
   )
), sep = "\t")
```
...
# filter as in plot
nodes_ann <- nodes_ann[expression_fc>1.4]
nodes_ann[is.na(active_TF), active_TF := FALSE]
nodes_ann <- nodes_ann[active_TF == TRUE]

# add labels
nodes_ann[, label := ""]
nodes_ann[common_name != "", label := common_name]
nodes_ann[label == "", label := gene_name]
nodes_ann[label == "", label := stringr::str_trunc(og, 15)]
nodes_ann[nchar(label) > 30,  label := stringr::str_trunc(label, 15)]
...
pdf(file.path(
  fig_dir, sprintf(
    "top_genes_metrics_%s.pdf", fn
  )
), height = 8, width = 21)

classes <- unique(nodes_ann$cell_type)
classes <- c("cnidocyte", "neuron_GATA_Islet", "neuron_Pou4_FoxL2", "gland")
for (ct in classes) {
  ...
  plot_list <- lapply(c(
    ...
    ggplot(
      ...
      scale_fill_gradientn(
        ...
        colours = c(
          "#ffd195", "#ffd39c", "#ffa400", "#e33a04",
          "#a61414", "#62083f", "#490446"
        )
      ) +
...
Compare network metrics across cell types

```{r}
nodes_ann_cts <- unique(nodes_ann[, .(
  gene, cell_type,
    degree_centrality_all,
    betweenness_centrality,
    eigenvector_centrality,
...
nodes_ann_cts[, gene := NULL]
nodes_ann_cts <- melt.data.table(nodes_ann_cts, id.vars = c("cell_type"))
nodes_ann_cts[, cell_type_box := paste0(cell_type, "_box")]

gp_nodes_ann_cts <- ggplot(nodes_ann_cts, aes(cell_type, value, fill = cell_type)) +
  geom_boxplot() +
  facet_wrap(~ variable, scales = "free_y", ncol = 1) +
  scale_fill_manual(values = c(
    cts_cols, structure(
...
ggsave(
  file.path(fig_dir, sprintf("top_genes_metrics_compara_%s.pdf", fn)), 
  gp_nodes_ann_cts, 
  height = 12, width = 8
)
...
For all TF, calculate an "overlapping index" (N cell-specific targets / total targets)

```{r}
# in how many cell types a TF is in GRN
grn_ntg <- unique(
  unique(grn_ct_dt[, .(cell_type, gene, target_gene)])
)
...
# add metadata
tfs_target_self <- unique(grn_ct_dt[target_self == TRUE]$gene)
...
Plot diversity index vs overlap index
...
ind_fn <- file.path(fig_dir, sprintf("GRN_ovl_vs_div_index_%s.pdf", fn))
ggsave(
...
```{r}
grn_comp <- merge.data.table(grn_comp, gold, by = "gene", all.x = TRUE, sort = FALSE)
grn_comp[, n_cell_types := factor(n_cell_types, levels = 1:max(grn_comp$n_cell_types))]

ovl_n_gp <- ggplot(grn_comp) +
  geom_violin(
    aes(n_cell_types, overlap_index),
    scale = "width"
  ) +
  ggbeeswarm::geom_quasirandom(
    data = grn_comp[!is.na(common_name)],
    aes(n_cell_types, overlap_index, size = target_genes),
    shape = 21, alpha = 1, fill = "#8a8a8a", color = "#3d3d3d"
  ) +
  labs(x = "number of cell types", y = "overlap index")

ggsave(
  str_replace(ind_fn, ".pdf", "_num_cell_types.pdf"),
  ovl_n_gp,
  width = 8, height = 4
)
...
fwrite(
  grn_comp,
  file.path(
    grn_res_dir, sprintf("GRN_ovl_vs_div_index_%s.tsv", fn)
  ),
  sep = "\t"
)
...
pdf(file.path(
  fig_dir,
  sprintf("GRN_ovl_vs_div_index_%s_eulers.pdf", fn)
), height = 5, width = 10)

# select TF
#for (g in unique(grn_comp[gene %in% dt$gene][order(diversity_index)]$gene)) {
for (g in grn_comp[gene %in% dt$gene][order(diversity_index)][gene %in% gold$gene]$gene) {

  if (g %in% gold$gene) {
    gnm <- gold[gene == g]$common_name
  } else {
  ...
  if (nrow(dtg) > 1) {
    ...
    # euler diagram
    top_cts <- sort(apply(mat, 2, sum), decreasing = TRUE)
    fit <- euler(mat[, names(top_cts), drop = FALSE])
    p1 <- plot(
      fit,
      quantities = TRUE,
      labels = list(font = 4),
