...
for (ct in cell_types) {
  ...
  grn_ct[, target_accessibility := max(target_accessibility), .(gene, target_gene, cell_type, stage)]
  grn_ct[, peak_tf_correlation_score := max(peak_tf_correlation_score), .(gene, target_gene, cell_type, stage)]
  grn_ct[, in_silico_chip_score := max(in_silico_chip_score), .(gene, target_gene, cell_type, stage)]
...
#saveWorkbook(
#  wb_peaks,
#  file.path(grn_dir, "networks", sprintf("grn_global_peaks_%s.xlsx", fn)),
#  overwrite = TRUE
#)
saveWorkbook(
  wb_genes,
  file.path(grn_dir, "networks", sprintf("grn_global_genes_%s.xlsx", fn)),
  overwrite = TRUE
)
```
...
cart_list <- lapply(cell_types, function(ct) {
