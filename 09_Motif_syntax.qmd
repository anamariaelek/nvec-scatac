...
for ct in cnidocyte cnidocyte_gastrula ecto_pharynx ectoderm ecto_aboral EMS EMS_ecto_boundary
do
Rscript 09_Motif_syntax_bottom_up.R $ct $ovl > logs/09_Motif_syntax_$ct.log 2>&1 &
done


for ct in gastro_circular_muscle_1 gastro_circular_muscle_2 gastro_parietal_muscle gastro_IRF1_2 gastro_somatic_gonad muscle_tentacle_retractor muscle_mesentery_retractor
do
Rscript 09_Motif_syntax_bottom_up.R $ct $ovl > logs/09_Motif_syntax_$ct.log 2>&1 &
done


for ct in digestive_filaments_1 digestive_filaments_2 digestive_filaments_3 epidermis_1 epidermis_2 precursors_PGC precursors_endoNPC precursors_NPC
do
Rscript 09_Motif_syntax_bottom_up.R $ct $ovl > logs/09_Motif_syntax_$ct.log 2>&1 &
done


for ct in NPC_1 NPC_2 neuron_GATA_Islet_1 neuron_GATA_Islet_2 neuron_Pou4_FoxL2_1 neuron_Pou4_FoxL2_2 neuron_Pou4_FoxL2_3 neuronal_gastrula gland gland_mucin
do
Rscript 09_Motif_syntax_bottom_up.R $ct $ovl > logs/09_Motif_syntax_$ct.log 2>&1 &
done
```
...
# save plots
pdf(
  file.path(fig_dir, "motif-hits-ranges-reduced-padj.pdf"), 
  width = 24, height = 18
)
print(plot_hits_lst)
dev.off()
...
# load reduced motif hits (top-down)
...
# load reduced motif hits (bottom-up)
mts_ct <- fread(file.path(syn_dir, "motif-hits-selected.tsv.gz"))
#mts_ct <- fread(file.path(syn_dir, "motif-hits-all.tsv.gz"))
...
arc_fam_col <- structure(
  colorRampPalette(brewer.pal(8, "Dark2"))(length(arc_fam_vec)),
  names = arc_fam_vec
)
...
ggsave(
  file.path(fig_dir, "motif-hits-number-fc-dotmap-all.pdf"), 
