...
# filter peaks by in silico chip score
...
```{r}
# load results
syn_sub_dir <- "q0.95-hits-enrich-assign-motifs-ovl-0.5"
res_fns <- list.files(
...
syn_sub_dir <- "q0.9-hits-enrich-assign-motifs-ovl-0.5"
...
```{r}
# peaks counts
mts_ct_pks <- unique(cmb_dt[, .(cell_type, peak, iteration)])

# motif counts
mts_ct_mts <- unique(cmb_dt[, .(cell_type, motif, iteration)])
