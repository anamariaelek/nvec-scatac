...
syn_sub_dirs <- c(
  "enrich" = "fcenrovl0.5",
  "enrich | assign" = "fcenrassignovl0.5",
  #"enrich | assign | ischip" = "fcenrassignovl0.5ics",
  "active" = "fcactovl0.5"
)
...
for (it in syn_sub_dirs) {
  ...
  use_genes_assigned_to_motfs <- switch(
    it,
    "fcenrovl0.5" = FALSE,
    "fcenrassignovl0.5" = FALSE,
    "fcenrassignovl0.5ics" = TRUE,
    "fcactovl0.5" = TRUE
  )
...
cmb_dt[, motif_frac_nocp_occur := motif_nocp_occur / n_nocp_pks]

# save
fwrite(
  cmb_dt, 
  file.path(syn_dir, "motif-hits-combinations.tsv.gz"), 
  sep = "\t"
)
```
...
```{r}
cmb_dt <- fread(file.path(syn_dir, "motif-hits-combinations.tsv.gz"))
...
for (stage in c("adult", "gastrula")) {
  ...
  # save plot
  pdf(
    file.path(fig_dir, sprintf("compare-peaks-per-cell-type-%s.pdf", stage)), 
    width = 24, height = 26
  )
  print(ggp_ct)
  dev.off()
...
Plot top motifs combinations for each iteration
...
for (it in syn_sub_dirs) {
    ...
    lr <- lapply(seq_len(is), function(i) {
      ...
      lg <- lapply(ms, function(mt) {
        ...
        if (mm %in% arc_as$archetype_name) {
          mg <- paste(arc_as[archetype_name == mm]$gene_label, collapse = " |\n")
        }
  ...
  # save plot
  pdf(
    file.path(fig_dir, sprintf("motifs-combinations-ranked-%s.pdf", it)), 
    width = 6, height = 24
  )
  print(cl)
  dev.off()
