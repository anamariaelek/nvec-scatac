...
# load reduced motif hits
mts_ct <- fread(file.path(syn_dir, "motif-hits-selected.tsv.gz"))
mts_ct[, archetype_name := motif]

# # # # # # # # # # # # # # # #
#  motif enrichment metadata  #
# # # # # # # # # # # # # # # #
...
ggsave(
  file.path(fig_dir, "motif-hits-number-fc-dotmap-insilico-chip.pdf"), 
...
for (ct in cell_types) {
  ...
  message(ct)
  cols <- c("N", "peak", "promoter", "motif", "motif_occur")
  ...
  if (nrow(mts_com) > 0) { 
    ...
    mts_top <- rbindlist(list(
      mts_top[, ..cols],
      mts_com[, ..cols]
    ), use.names = TRUE)[, cell_type := ct][, n_pks := n_pks]
  }
...
# save plots
pdf(
  file.path(fig_dir, "motif-TF-occurance-per-cell-type-insilico-ChIP.pdf"), 
  width = 24, height = 42
)
