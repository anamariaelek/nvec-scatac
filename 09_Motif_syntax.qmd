...
Motif lexicons analysis:  
  - get locations of all hits (motif scores above the 95th quantile of genomic binding energy)  
    for cell-type enriched motifs  
  - get non-overlapping motifs hits  
  - get motif hits with minimum distance between motifs (?)
  
First load the motifs hits input data.
...
Next we will reduce sets of overlapping motif hits in peaks to one best scoring 
...
do
Rscript 09_Motif_syntax.R $chr $ovl > logs/09_Motif_syntax_$chr.log 2>&1 &
done
...
Per cell type lexicons: in cell type specific peaks

```{r}
# load motifs hits
#select_hits <- fread(file.path(syn_dir, "motif-hits-all.tsv.gz"))
select_hits <- fread(file.path(syn_dir, "motif-hits-selected.tsv.gz"))
...
fwrite(
  mts_ct, 
  #file.path(syn_dir, "motif-hits-all-cell-type.tsv.gz"),
...
Count motif hits in peaks per cell type.
...
```{r}
# load results
# mts_ct <- fread(file.path(syn_dir, "motif-hits-all-cell-type.tsv.gz"))
mts_ct <- fread(file.path(syn_dir, "motif-hits-selected-cell-type.tsv.gz"))

# get motif counts in peaks per cell type
cmb_ls <- lapply(cell_types, function(ct) {
...
cpl_ls <- lapply(cell_types, function(ct) {
  ...
  while (length(unique(cmb_ct$motif)) > 100) {
    ...
    if (length(unique(cmb_mt$motif)) < 50)
      break
    cmb_ct <- cmb_mt
    print(sprintf("occurance > %s: %s motifs", 10*i, length(unique(cmb_mt$motif))))
    i <- i + 1
  }
  ...
  cpl_ct <- ggplot(cmb_ct, aes(motif_occur, motif)) + 
    ...
    scale_x_continuous(expand = expansion(mult = c(0, 0.1)), trans = "log10") +
    labs(title = ct) +
    theme(
