...
# all genes
genes_gr <- read_gtf("genome/Nvec_v4_merged_annotation_sort.gtf")
...
# genes
genes <- read_gtf(
  "genome/Nvec_v4_merged_annotation_sort.gtf",
  as_GRanges = TRUE
)
...
```
```{r}
#| label: scatac_peaks_debug
#| ehco: FALSE
#| eval: FALSE
#| include: FALSE

# DO NOT RUN
# re-use ArchR files above instead

# peaks assignment to cell types for adult and gastrula
pks_flt <- rbindlist(lapply(cell_types, function(ct) {
  
  print(ct)

  # gastrula or adult cell type
  stage <- if (ct %in% adult_cell_types) {
     "adult"
  } else {
     "gastrula"
  }
  stage_dir <- switch(stage, "adult" = adult_dir, "gastrula" = gastr_dir)

  # load peaks
  pks_ct_fn <- file.path(
    stage_dir, 
    "ArchRProj", "Peaks", "cell_type_filtered", 
    sprintf("Peaks-%s.bed", ct)
  )
  pks_ct_dt <- fread(pks_ct_fn)[, 1:3]
  setnames(pks_ct_dt, c("seqnames", "start", "end"))
  pks_ct_dt[, cell_type := ct][, stage := stage]
    
}), fill = TRUE, use.names = TRUE)

setcolorder(pks_flt, c("seqnames", "start", "end", "cell_type"))

# map peaks
query_gr <- makeGRangesFromDataFrame(
    pks_flt, keep.extra.columns = TRUE
)
subject_gr <- makeGRangesFromDataFrame(
    pks_dt, keep.extra.columns = TRUE
)
...
pks_cts <- pks_flt[queryHits(ovl)]$cell_type
pks_cts_dt <- pks_dt[subjectHits(ovl)]
pks_cts_dt$cell_type <- pks_cts
pks_cts_dt <- unique(pks_cts_dt)

# order
pks_cts_dt[, stage := ifelse(cell_type %in% adult_cell_types, "adult", "gastrula")]
pks_cts_dt[, stage := paste(sort(unique(.SD$stage)), collapse = ","), by = "peak"]
pks_cts_dt[, cell_type := factor(cell_type, levels = cell_types)]
setorder(pks_cts_dt, "seqnames", "start", "end", "cell_type")
setcolorder(pks_cts_dt, c("seqnames", "start", "end", "peak", "cell_type"))
pks_cts_dt <- unique(pks_cts_dt)
...
```{r}
#| label: chip_peaks_debug
#| ehco: FALSE
#| eval: FALSE
#| include: FALSE

# to save time while debugging, pass overlap pairs between chip and atac peaks directly to classifier
...
seqlevels(all_gr) <- seqlevels(BSgenome.jaNemVect1.1.DToL.Assembly)
all_gr <- sort(all_gr)
...
```{r}
#| label: classify_promoters
#| ehco: FALSE
#| eval: FALSE
#| include: FALSE

# DO NOT RUN
# we will classify promoters separately for adult and gastrula stages below
...
saveRDS(
  res_list,
  file.path(pks_dir, "annotaton_promoters.rds")
)
```
```{r}
class_cols <- c(
...
# save
fwrite(peaks_res, file.path(pks_dir, "annotaton_promoters_decison_tree.tsv"))
...
fwrite(
  peaks_annotation,
  file.path(pks_dir, "annotaton_peaks.tsv"),
  sep="\t", col.names = TRUE
)
...
fwrite(
  genes_promoters,
  file.path(pks_dir, "annotaton_promoters.tsv"),
  sep="\t", col.names = TRUE
)
...
```{r}
# load peas classification
pks_dt <- fread(file.path(pks_dir, "annotaton_promoters_decison_tree.tsv"))
...
stopifnot(length(pks_dt[,.N,cell_type][!cell_type %in% gen_fc$cell_type]$cell_type) == 0)
...
pks_dt <- unique(pks_dt[, .(peak, cell_type, stage, gene, gene_expression_fc, gene_tau_index, promoter)])

# load promoters per gene
peaks_annotation <- fread(file.path(pks_dir, "annotaton_peaks.tsv"))
...
gp_tau_1 <- ggplot(
  ...
  scale_y_continuous(
    limits = c(0, quantile(cre_dt$n_peaks_per_gene_per_cell_type, 0.999)),
    oob = scales::squish
  ) +
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
