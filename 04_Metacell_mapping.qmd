...
# peak metadata
peak_dt <- fread(file.path(pks_dir, "Peaks_cell_type_mapped.tsv"))
...
```{r}
# Combine gene score data for adult and gastrula
# if (lvl == "metacell") {
#     acc_adult <- adult_gene_access
#     acc_gastr <- gastr_gene_access
# } else if (lvl == "cell_type") {
#     acc_adult <- adult_gene_access_ct
#     acc_gastr <- gastr_gene_access_ct
# } else if (lvl == "single_cell") {
#     acc_adult <- adult_gene_access_sc
#     acc_gastr <- gastr_gene_access_sc
#     acc_asign <- rbindlist(list(
#         "adult" = as.data.table(proj_adult@cellColData)[,.(Cell, SEACell)],
#         "gastrula" = as.data.table(proj_gastr@cellColData)[,.(Cell, SEACell)]
#     ), idcol = "stage")
#     acc_asign[, SEACell := paste(stage, SEACell, sep = "_")]
#     acc_asign[, Cell := paste(stage, Cell, sep = "_")]
#     fwrite(acc_asign, file.path(
#         res_dir, lvl, "SEACell_assignment.tsv"
#     ), sep = "\t")
# }
# 
# colnames(acc_gastr) <- paste0("gastrula_", colnames(acc_gastr))
# colnames(acc_adult) <- paste0("adult_", colnames(acc_adult))
# 
# gns <- unique(c(rownames(acc_gastr), rownames(acc_adult)))
# 
# miss_gastr_gns <- setdiff(gns, rownames(acc_gastr))
# miss_gastr_mat <- matrix(
#     0,
#     nrow = length(miss_gastr_gns),
#     ncol = ncol(acc_gastr),
#     dimnames = list(miss_gastr_gns, colnames(acc_gastr))
# )
# acc_gastr <- do.call(rbind, list(acc_gastr, miss_gastr_mat))
# 
# miss_adult_gns <- setdiff(gns, rownames(acc_adult))
# miss_adult_mat <- matrix(
#     0,
#     nrow = length(miss_adult_gns),
#     ncol = ncol(acc_adult),
#     dimnames = list(miss_adult_gns, colnames(acc_adult))
# )
# acc_adult <- do.call(rbind, list(acc_adult, miss_adult_mat))
# 
# acc_mt <- do.call(cbind, list(acc_gastr[gns, ], acc_adult[gns, ]))
# saveRDS(
#     acc_mt,
#     file.path(
#         res_dir, lvl, sprintf("gene_scores_%s.rds", lvl)
#     )
# )
```
